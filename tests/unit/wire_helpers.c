...
static void check_arity_roundtrip(IdmArity arity, const char *name) {
    ...
    check(buf.len == 9u, "arity wire width");
    IdmByteReader r;
    idm_byte_reader_init(&r, (const unsigned char *)buf.data, buf.len);
...
static void check_arity_wire(void) {
    ...
    check_arity_roundtrip(idm_arity_exact(64u), "arity exact");
}

static void check_invalid_arity_kind(void) {
    unsigned char data[9] = { 99u };
    IdmByteReader r;
    idm_byte_reader_init(&r, data, sizeof(data));
    IdmError err;
    idm_error_init(&err);
...
static void check_truncated_arity(void) {
    unsigned char data[] = { (unsigned char)IDM_ARITY_RANGE, 0u };
    IdmByteReader r;
    idm_byte_reader_init(&r, data, sizeof(data));
    IdmError err;
    idm_error_init(&err);
...
static void check_invalid_arity_bounds(void) {
    IdmBuffer buf;
    idm_buf_init(&buf);
    IdmError err;
    idm_error_init(&err);
    check(!idm_arity_serialize(&buf, idm_arity_range(4u, 2u), &err), "invalid arity bounds rejected");
    check(err.present, "invalid arity bounds state");
    idm_error_clear(&err);
    idm_buf_destroy(&buf);
}
...
static void check_invalid_invocation_mask(void) {
    ...
    IdmError err;
    idm_error_init(&err);
    check(!idm_callable_contract_serialize(&buf, &contract, &err), "invocation mask overflow rejected");
    check(err.present && err.message && strstr(err.message, "exceeds signature arity") != NULL, "invocation mask overflow message");
    idm_error_clear(&err);
    idm_buf_destroy(&buf);
...
int idm_unit_wire_helpers(void) {
    ...
    check_invalid_arity_bounds();
    check_invalid_invocation_mask();
    return 0;
}
