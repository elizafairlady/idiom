...
static void test_source_operator_surface(void) {
    ...
    check_operator_eval(&rt, "defn negate x -> sub 0 x\noperator negof precedence: 80 fixity: prefix -> negate\nadd 50 (negof 8)\n", NULL, 42);
...
static void test_source_operator(void) {
    ...
    core = NULL;
    CHECK(ish_expand_string(&rt, "<operator-expand-test>", "if (2 <= 2) do 42 else do 0 end\n", &core, &err));
    ish_bc_init(&module);
    CHECK(ish_core_compile_main(core, &module, &main_fn, &err));
    CHECK(ish_vm_run(&rt, &module, main_fn, &out, &err));
    ...
    core = NULL;
    CHECK(ish_expand_string(&rt, "<operator-expand-test>", "add (div 80 2) (mod 5 3)\n", &core, &err));
    ish_bc_init(&module);
    CHECK(ish_core_compile_main(core, &module, &main_fn, &err));
    CHECK(ish_vm_run(&rt, &module, main_fn, &out, &err));
    CHECK(out.tag == ISH_VAL_INT && out.as.i == 42);
    ...
    core = NULL;
    CHECK(ish_expand_string(&rt, "<operator-expand-test>", "add (pow 6 2) 6\n", &core, &err));
    ish_bc_init(&module);
    CHECK(ish_core_compile_main(core, &module, &main_fn, &err));
    CHECK(ish_vm_run(&rt, &module, main_fn, &out, &err));
    CHECK(out.tag == ISH_VAL_INT && out.as.i == 42);
    ...
    core = NULL;
    CHECK(!ish_expand_string(&rt, "<operator-expand-test>", "add 1 nope\n", &core, &err));
    CHECK(err.present);
    ish_error_clear(&err);
...
static void test_operator_macro_negatives(void) {
    ...
    expect_expand_error_rt(&rt, "<op-thread-prefix>", "operator bad fixity: prefix -> thread-first\n", "thread operators must be infix");
