...
static void test_tuple_dag(IdmRuntime *rt, IdmError *err) {
    ...
    check(idm_pattern_selector_info(rt, selector, &info, err), "selector info build");
    check_error(err, "selector info error");
    check(idm_is_dict(info), "selector info is a dict");
    IdmValue nodes = idm_nil();
    check(idm_dict_get(info, idm_atom(rt, "nodes"), &nodes), "selector info nodes key");
    size_t node_entries = 0;
    for (IdmValue cur = nodes; idm_is_pair(cur); cur = idm_cdr(cur, err)) node_entries++;
    check(node_entries == idm_pattern_selector_node_count(selector), "selector info node parity");
    IdmValue arities = idm_nil();
    check(idm_dict_get(info, idm_atom(rt, "arities"), &arities), "selector info arities key");
