...
static void test_expander_surface_removed(void) {
    IdmRuntime rt;
    idm_runtime_init(&rt);
    expect_expand_error_rt(&rt, "<surface-removed>",
        "expander-surface :operators\n",
        "unbound identifier 'expander-surface'");
    idm_runtime_destroy(&rt);
}
...
static void test_package_env_diagnostics(void) {
    const uint32_t truncated_push[] = {
        (uint32_t)IDM_OP_PUSH_PACKAGE_ENV,
    };
    expect_bytecode_verify_error("<package-env-push-truncated>", truncated_push, sizeof(truncated_push) / sizeof(truncated_push[0]), "PUSH_PACKAGE_ENV missing operand");
    ...
    CHECK(err.present);
    CHECK(err.message != NULL);
    if (err.message && !strstr(err.message, "POP_PACKAGE_ENV with no active package env")) {
        fprintf(stderr, "FAIL <package-env-pop-empty>: expected runtime error containing \"POP_PACKAGE_ENV with no active package env\", got \"%s\"\n", err.message);
        failures++;
    }
    idm_error_clear(&err);
    idm_bc_destroy(&module);
...
void run_diagnostics_suite(void) {
    ...
    test_expander_surface_removed();
    test_minus_reader_diagnostics();
    test_package_env_diagnostics();
}
