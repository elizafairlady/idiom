...
static void check_classified(IdmReaderArtifact *artifact, const char *raw, IdmSyntaxKind kind, const char *text, int64_t integer, double real, IdmError *err, const char *name) {
    ...
    check_ok_or_error(idm_reader_read_terms_string("reader_escape", raw, 0, &transparent, NULL, err), err, "classifier transparent read");
    check(!err->present, "classifier transparent error clear");
    check_ok_or_error(idm_reader_read_artifact_string(artifact, "reader_escape", raw, 0, &custom, NULL, err), err, "classifier artifact read");
...
int idm_unit_reader_escape(void) {
    ...
    IdmSyntax *syn = NULL;
    check(!idm_reader_read_terms_string("reader_escape", "\"x\\q\"", 0, &syn, NULL, &err), "transparent reject");
    ...
    check(!idm_reader_read_artifact_string(artifact, "reader_escape", "\"x\\q\"", 0, &syn, NULL, &err), "artifact string reject");
    ...
    check(!idm_reader_read_artifact_string(artifact, "reader_escape", "S\\q", 0, &syn, NULL, &err), "artifact interp reject");
