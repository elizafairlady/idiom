...
static bool prim_append(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    return idm_list_append(rt, args[0], args[1], out, err);
}
...
static bool expand_home_path(IdmRuntime *rt, IdmExec *exec, char **path_io, IdmError *err) {
    char *path = *path_io;
    if (path[0] != '~' || (path[1] != '\0' && path[1] != '/')) return true;
    const char *home = idm_exec_env_get(exec, "HOME");
    if (!home) {
        home = getenv("HOME");
        idm_phase_env_record(rt, "HOME", home);
    }
...
static bool prim_cd(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    if (!expand_home_path(rt, exec, &path, err)) { free(path); return false; }
    char *candidate = NULL;
    if (path[0] == '/') {
        candidate = path;
        path = NULL;
...
static bool prim_env_get(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    if (!value) {
        value = getenv(name);
        idm_phase_env_record(rt, name, value);
    }
    free(name);
    if (!value) {
        *out = idm_nil();
...
static bool prim_print_to(IdmRuntime *rt, FILE *stream, const IdmValue *args, uint32_t argc, bool newline, IdmValue *out, IdmError *err) {
    ...
    for (uint32_t i = 0; i < argc; i++) {
        ...
        if (!idm_value_append_display(&buf, args[i])) { idm_buf_destroy(&buf); return idm_error_oom(err, idm_span_unknown(NULL)); }
    }
...
static int cmp_dict_total(IdmValue a, IdmValue b) {
    ...
    int r = cmp_size(ac, bc);
    if (r != 0 || ac == 0) return r;
    IdmDictEntry *a_entries = calloc(ac, sizeof(*a_entries));
    IdmDictEntry *b_entries = calloc(bc, sizeof(*b_entries));
    if (!a_entries || !b_entries || !dict_collect_entries(a, a_entries, ac) ||
        !dict_collect_entries(b, b_entries, bc)) {
        free(a_entries);
        free(b_entries);
        return 0;
    }
    ...
    for (size_t i = 0; r == 0 && i < ac; i++) r = cmp_value_total(a_entries[i].key, b_entries[i].key);
    for (size_t i = 0; r == 0 && i < ac; i++) r = cmp_value_total(a_entries[i].value, b_entries[i].value);
    free(a_entries);
    free(b_entries);
    return r;
}
...
static bool tag_is_number(IdmValueTag t) {
    return t == IDM_VAL_INT || t == IDM_VAL_BIGNUM || t == IDM_VAL_FLOAT;
}

static int cmp_number_total(IdmValue a, IdmValue b) {
    IdmValueTag ta = idm_value_tag(a), tb = idm_value_tag(b);
    if (ta != IDM_VAL_FLOAT && tb != IDM_VAL_FLOAT) return idm_int_compare(a, b);
    double x = 0.0, y = 0.0;
    num_as_double(a, &x);
    num_as_double(b, &y);
    if (isnan(x) || isnan(y)) {
    ...
    if (x < y) return -1;
    if (x > y) return 1;
    return ta == tb ? 0 : (ta == IDM_VAL_FLOAT ? 1 : -1);
}

static int cmp_value_total(IdmValue a, IdmValue b) {
    IdmValueTag ta = idm_value_tag(a), tb = idm_value_tag(b);
    if (ta != tb) {
        bool na = tag_is_number(ta), nb = tag_is_number(tb);
        if (na && nb) return cmp_number_total(a, b);
        int ra = na ? (int)IDM_VAL_INT : (int)ta;
        int rb = nb ? (int)IDM_VAL_INT : (int)tb;
        return ra < rb ? -1 : 1;
    }
    switch (ta) {
        case IDM_VAL_NIL:
        ...
        case IDM_VAL_TUPLE: {
            int r = cmp_size(idm_sequence_count(a), idm_sequence_count(b));
            return r != 0 ? r : cmp_sequence_total(a, b);
        }
        case IDM_VAL_VECTOR:
...
static IdmBuiltinType type_pred_type_kind(IdmPrimitive prim) {
    switch (prim) {
        ...
        case IDM_PRIM_BITS_P: return IDM_BUILTIN_TYPE_BITSTRING;
...
static bool prim_json_string(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    IdmBuffer buf;
    idm_buf_init(&buf);
    if (!idm_value_write_json(&buf, args[0])) {
        idm_buf_destroy(&buf);
        return idm_error_oom(err, idm_span_unknown(NULL));
    }
...
static bool prim_selector_info(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    IdmValue subject = args[0];
    if (idm_is_closure(subject)) {
        const IdmPatternSelector *selector = idm_closure_selector(subject);
        if (!selector) {
            IdmDictEntry items[1];
            items[0].key = idm_atom(rt, "direct");
            items[0].value = idm_bool(rt, true);
            *out = idm_dict(rt, items, 1u, err);
            return !(err && err->present);
        }
        return idm_pattern_selector_info(rt, selector, out, err);
    }
    if (idm_is_regex(subject)) {
        IdmRegex *rx = idm_regex_value_get(subject, err);
        if (!rx) return false;
        return idm_byteprog_info(rt, idm_regex_byteprog(rx), out, err);
    }
    return idm_primitive_type_error(rt, err, "selector-info", subject, "a closure or a regex");
}
...
static bool prim_str_len(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_STRING) return idm_primitive_type_error(rt, err, "byte-len", args[0], "a string");
...
static bool prim_str_slice(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_STRING) return idm_primitive_type_error(rt, err, "byte-slice", args[0], "a string");
    size_t len = idm_string_length(args[0]);
    int64_t a = 0;
    int64_t b = 0;
    if (!value_to_i64(args[1], &a)) return idm_primitive_type_error(rt, err, "byte-slice", args[1], "integer bounds");
    if (!value_to_i64(args[2], &b)) return idm_primitive_type_error(rt, err, "byte-slice", args[2], "integer bounds");
    if (a < 0 || b < a || (uint64_t)b > len) {
        idm_error_set(err, idm_span_unknown(NULL), "byte-slice range %lld..%lld out of bounds for length %zu", (long long)a, (long long)b, len);
        return idm_error_reason(rt, err, "slice-out-of-range", 3, args[1], args[2], idm_int((int64_t)len));
    }
    *out = idm_string_slice_value(rt, args[0], (size_t)a, (size_t)b, err);
    return !(err && err->present);
}

static bool prim_str_find_range(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    const char *s; size_t len;
    if (!idm_primitive_require_string_arg(rt, args[0], &s, &len, "byte-find-range", err)) return false;
    int64_t lo = 0, hi = 0, start = 0;
    if (!value_to_i64(args[1], &lo) || lo < 0 || lo > 255) return idm_primitive_type_error(rt, err, "byte-find-range", args[1], "a byte value");
    if (!value_to_i64(args[2], &hi) || hi < 0 || hi > 255) return idm_primitive_type_error(rt, err, "byte-find-range", args[2], "a byte value");
    if (!value_to_i64(args[3], &start)) return idm_primitive_type_error(rt, err, "byte-find-range", args[3], "an integer start");
    size_t from = (size_t)clamp_index(start, len);
    for (size_t i = from; i < len; i++) {
        unsigned char b = (unsigned char)s[i];
        if (b >= (unsigned char)lo && b <= (unsigned char)hi) {
...
static bool prim_str_find(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    if (!idm_primitive_require_string_arg(rt, args[0], &s, &len, "byte-find", err)) return false;
    if (!idm_primitive_require_string_arg(rt, args[1], &needle, &nlen, "byte-find", err)) return false;
    int64_t start = 0;
    if (!value_to_i64(args[2], &start)) return idm_primitive_type_error(rt, err, "byte-find", args[2], "an integer start");
    size_t from = (size_t)clamp_index(start, len);
...
static bool prim_str_byte(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_STRING) return idm_primitive_type_error(rt, err, "byte", args[0], "a string");
    int64_t i = 0;
    if (!value_to_i64(args[1], &i)) return idm_primitive_type_error(rt, err, "byte", args[1], "an integer index");
    if (i < 0) { *out = idm_nil(); return true; }
    int b = idm_string_byte_at(args[0], (size_t)i);
    *out = b < 0 ? idm_nil() : idm_int((int64_t)b);
    return true;
}

static bool prim_str_line_of(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_STRING) return idm_primitive_type_error(rt, err, "line-of", args[0], "a string");
    int64_t off = 0;
    if (!value_to_i64(args[1], &off)) return idm_primitive_type_error(rt, err, "line-of", args[1], "an integer offset");
    size_t len = idm_string_length(args[0]);
    if (off < 0 || (uint64_t)off > len) {
        idm_error_set(err, idm_span_unknown(NULL), "line-of offset %lld out of bounds for length %zu", (long long)off, len);
        return idm_error_reason(rt, err, "slice-out-of-range", 2, args[1], idm_int((int64_t)len));
    }
    *out = idm_int((int64_t)idm_string_newlines_before(args[0], (size_t)off));
    return true;
}

static bool prim_str_line_start(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_STRING) return idm_primitive_type_error(rt, err, "line-start", args[0], "a string");
    int64_t line = 0;
    if (!value_to_i64(args[1], &line)) return idm_primitive_type_error(rt, err, "line-start", args[1], "an integer line");
    if (line < 0) { *out = idm_nil(); return true; }
    size_t off = 0;
    *out = idm_string_line_start(args[0], (size_t)line, &off) ? idm_int((int64_t)off) : idm_nil();
    return true;
}
...
static bool bits_opt_atom(IdmRuntime *rt, const char *name, IdmValue v, bool *little, bool *is_signed, bool *saw_endian, bool *saw_signed, IdmError *err) {
    if (idm_value_tag(v) == IDM_VAL_ATOM) {
        const IdmSymbol *sym = idm_value_symbol(v);
        if (sym == rt->bits_sym_big || sym == rt->bits_sym_little) {
            if (*saw_endian) return idm_error_set(err, idm_span_unknown(NULL), "%s endianness is set twice", name);
            *saw_endian = true;
            *little = sym == rt->bits_sym_little;
            return true;
        }
        if (is_signed && (sym == rt->bits_sym_unsigned || sym == rt->bits_sym_signed)) {
            if (*saw_signed) return idm_error_set(err, idm_span_unknown(NULL), "%s signedness is set twice", name);
            *saw_signed = true;
            *is_signed = sym == rt->bits_sym_signed;
            return true;
        }
    }
    return idm_primitive_type_error(rt, err, name, v, is_signed ? ":big, :little, :signed, or :unsigned" : ":big or :little");
}

static bool bits_opts(IdmRuntime *rt, const char *name, IdmValue opts, bool *little, bool *is_signed, IdmError *err) {
    *little = false;
    if (is_signed) *is_signed = false;
    bool saw_endian = false, saw_signed = false;
    if (idm_is_nil(opts)) return true;
    if (idm_value_tag(opts) == IDM_VAL_ATOM) return bits_opt_atom(rt, name, opts, little, is_signed, &saw_endian, &saw_signed, err);
    if (idm_is_pair(opts)) {
        IdmValue cur = opts;
        while (idm_is_pair(cur)) {
            if (!bits_opt_atom(rt, name, idm_car(cur, NULL), little, is_signed, &saw_endian, &saw_signed, err)) return false;
            cur = idm_cdr(cur, NULL);
        }
        if (idm_is_nil(cur)) return true;
    }
    return idm_primitive_type_error(rt, err, name, opts, "an options atom or list");
}

static bool bits_range_args(IdmRuntime *rt, const char *name, IdmValue bits_v, IdmValue off_v, IdmValue len_v, uint64_t *off, uint64_t *len, IdmError *err) {
    if (!idm_is_bits(bits_v)) return idm_primitive_type_error(rt, err, name, bits_v, "a bitstring");
    int64_t o = 0, l = 0;
    if (!value_to_i64(off_v, &o)) return idm_primitive_type_error(rt, err, name, off_v, "an integer bit offset");
    if (!value_to_i64(len_v, &l)) return idm_primitive_type_error(rt, err, name, len_v, "an integer bit length");
    uint64_t total = idm_bits_len(bits_v);
    if (o < 0 || l < 0 || (uint64_t)o > total || (uint64_t)l > total - (uint64_t)o) {
        idm_error_set(err, idm_span_unknown(NULL), "%s range %lld+%lld out of bounds for %llu bits", name, (long long)o, (long long)l, (unsigned long long)total);
        return idm_error_reason(rt, err, "slice-out-of-range", 3, off_v, len_v, idm_int((int64_t)total));
    }
    *off = (uint64_t)o;
    *len = (uint64_t)l;
    return true;
...
static bool prim_bits_len(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (!idm_is_bits(args[0])) return idm_primitive_type_error(rt, err, "bits-len", args[0], "a bitstring");
    *out = idm_int_promote(rt, (int64_t)idm_bits_len(args[0]), err);
    return !(err && err->present);
}

static bool prim_bits_slice(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    uint64_t off = 0, len = 0;
    if (!bits_range_args(rt, "bits-slice", args[0], args[1], args[2], &off, &len, err)) return false;
    *out = idm_bits_slice(rt, args[0], off, len, err);
    return !(err && err->present);
}

static bool prim_bits_int(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    uint64_t off = 0, len = 0;
    bool little = false, is_signed = false;
    if (!bits_range_args(rt, "bits-int", args[0], args[1], args[2], &off, &len, err)) return false;
    if (!bits_opts(rt, "bits-int", args[3], &little, &is_signed, err)) return false;
    *out = idm_bits_int(rt, args[0], off, len, little, is_signed, err);
    return !(err && err->present);
}

static bool prim_bits_float(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    uint64_t off = 0, len = 0;
    bool little = false;
    if (!bits_range_args(rt, "bits-float", args[0], args[1], args[2], &off, &len, err)) return false;
    if (!bits_opts(rt, "bits-float", args[3], &little, NULL, err)) return false;
    if (len != 32u && len != 64u) return idm_primitive_type_error(rt, err, "bits-float", args[2], "a bit length of 32 or 64");
    IdmValue v = idm_bits_float(rt, args[0], off, len, little, err);
    if (err && err->present) return false;
    *out = float_result(rt, idm_float_value(v), err);
    return !(err && err->present);
}

static bool prim_bits_of_int(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (!idm_value_is_int(args[0])) return idm_primitive_type_error(rt, err, "bits-of-int", args[0], "an integer");
    int64_t len = 0;
    if (!value_to_i64(args[1], &len) || len < 0) return idm_primitive_type_error(rt, err, "bits-of-int", args[1], "a non-negative bit length");
    bool little = false, is_signed = false;
    if (!bits_opts(rt, "bits-of-int", args[2], &little, &is_signed, err)) return false;
    *out = idm_bits_of_int(rt, args[0], (uint64_t)len, little, err);
    return !(err && err->present);
}

static bool prim_bits_of_float(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_FLOAT) return idm_primitive_type_error(rt, err, "bits-of-float", args[0], "a float");
    int64_t len = 0;
    if (!value_to_i64(args[1], &len) || (len != 32 && len != 64)) return idm_primitive_type_error(rt, err, "bits-of-float", args[1], "a bit length of 32 or 64");
    bool little = false;
    if (!bits_opts(rt, "bits-of-float", args[2], &little, NULL, err)) return false;
    double d = idm_float_value(args[0]);
    unsigned char buf[8];
    size_t nbytes = (size_t)(len / 8);
    if (len == 32) {
        float f = (float)d;
        uint32_t u = 0;
        memcpy(&u, &f, 4u);
        for (size_t i = 0; i < 4u; i++) buf[i] = (unsigned char)(u >> (24u - 8u * i));
    } else {
        uint64_t u = 0;
        memcpy(&u, &d, 8u);
        for (size_t i = 0; i < 8u; i++) buf[i] = (unsigned char)(u >> (56u - 8u * i));
    }
    if (little) {
    ...
    *out = idm_bits_from_bytes(rt, buf, (uint64_t)len, err);
    return !(err && err->present);
}

static bool prim_bits_append(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (!idm_is_bits(args[0])) return idm_primitive_type_error(rt, err, "bits-append", args[0], "a bitstring");
    if (!idm_is_bits(args[1])) return idm_primitive_type_error(rt, err, "bits-append", args[1], "a bitstring");
    *out = idm_bits_append(rt, args[0], args[1], err);
    return !(err && err->present);
}

static bool prim_string_bits(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    const char *s = NULL;
    size_t len = 0;
    if (!idm_primitive_require_string_arg(rt, args[0], &s, &len, "string-bits", err)) return false;
    *out = idm_bits_from_bytes(rt, (const unsigned char *)s, (uint64_t)len * 8u, err);
    return !(err && err->present);
}

static bool prim_bits_string(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (!idm_is_bits(args[0])) return idm_primitive_type_error(rt, err, "bits-string", args[0], "a bitstring");
    uint64_t len = idm_bits_len(args[0]);
    if ((len & 7u) != 0u) {
        idm_error_set(err, idm_span_unknown(NULL), "bits-string requires a byte-aligned bitstring, got %llu bits", (unsigned long long)len);
        return idm_error_reason(rt, err, "not-byte-aligned", 1, idm_int((int64_t)len));
    }
    size_t nbytes = (size_t)(len / 8u);
    unsigned char *buf = malloc(nbytes ? nbytes : 1u);
    if (!buf) return idm_error_oom(err, idm_span_unknown(NULL));
    idm_bits_read(args[0], 0u, len, buf);
    *out = idm_string_n(rt, (const char *)buf, nbytes, err);
    free(buf);
    return !(err && err->present);
}
...
static bool prim_file_write(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    idm_phase_write_record(rt, path);
...
static bool prim_file_open(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    if (writable) idm_phase_write_record(rt, resolved);
...
static bool prim_file_list(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    char pb[PATH_MAX];
    if (!(path = resolve_cwd(path, pb, sizeof(pb)))) return result_error(rt, out, err);
    idm_phase_directory_record(rt, path);
...
static bool prim_total_allocs(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    (void)args; (void)err;
    *out = idm_int((int64_t)idm_heap_total_allocs(idm_active_heap(rt)));
    return true;
}

static bool prim_total_alloc_bytes(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    (void)args; (void)err;
    *out = idm_int((int64_t)idm_heap_total_alloc_bytes(idm_active_heap(rt)));
    return true;
}
...
static bool prim_record_update(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    *out = idm_record_update_value(rt, args[0], args[1], err);
    return !(err && err->present);
}

static bool prim_help(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    *out = idm_nil();
    if (!idm_is_closure(args[0])) return true;
    const IdmBytecodeModule *module = idm_closure_module(args[0]);
    uint32_t index = idm_closure_function_index(args[0]);
    if (!module || index >= module->function_count) return true;
    const char *doc = module->functions[index].doc;
    if (!doc || !doc[0]) return true;
    *out = idm_string(rt, doc, err);
    return !(err && err->present);
}
...
static bool prim_identifier_bound(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    IdmSyntax *syn = require_syntax(args[0], err);
    if (!syn) return false;
    if (!rt->identifier_bound) return idm_error_set(err, syn->span, "identifier-bound? is only available during macro expansion");
    bool bound = false;
    if (!rt->identifier_bound(rt->identifier_bound_user, rt, syn, &bound, err)) return false;
    *out = idm_atom(rt, bound ? "true" : "false");
    return true;
}
...
static bool require_session(IdmRuntime *rt, const char *name, IdmError *err) {
    ...
    if (!sched) return idm_error_set(err, idm_span_unknown(NULL), "%s requires an actor context", name);
    pthread_mutex_lock(&g_session_mu);
    bool ok = rt->repl != NULL || idm_repl_create_with(rt, sched, err) != NULL;
    pthread_mutex_unlock(&g_session_mu);
    return ok;
}

static bool prim_compile(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (!require_session(rt, "compile", err)) return false;
    size_t len = 0;
    const char *source = NULL;
    if (!idm_primitive_require_string_arg(rt, args[0], &source, &len, "compile", err)) return false;
...
static bool prim_abort(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (!require_session(rt, "abort", err)) return false;
    int64_t token = 0;
    if (!value_to_i64(args[0], &token) || token < 0) return idm_primitive_type_error(rt, err, "abort", args[0], "a compile token");
    idm_repl_abort(rt->repl, (uint64_t)token);
    *out = idm_atom(rt, "ok");
    return true;
}

static bool prim_session(IdmRuntime *rt, IdmValue *out) {
    (void)rt;
    IdmScheduler *sched = idm_exec_scheduler(idm_current_exec());
    uint64_t pid = sched ? idm_sched_session_pid(sched) : 0;
...
static bool prim_port_resize(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    if (idm_value_tag(args[0]) != IDM_VAL_PORT) return idm_primitive_type_error(rt, err, "port-resize", args[0], "a port");
    int64_t cols = 0;
    int64_t rows = 0;
    if (!require_nonnegative_int_arg(rt, err, "port-resize", args[1], &cols)) return false;
    if (!require_nonnegative_int_arg(rt, err, "port-resize", args[2], &rows)) return false;
    if (cols < 1 || cols > 65535) return idm_primitive_type_error(rt, err, "port-resize", args[1], "columns between 1 and 65535");
    if (rows < 1 || rows > 65535) return idm_primitive_type_error(rt, err, "port-resize", args[2], "rows between 1 and 65535");
    IdmScheduler *sched = job_sched("port-resize", err);
    if (!sched) return false;
    bool live = false;
    if (!idm_sched_port_resize(sched, idm_value_id(args[0]), (int)cols, (int)rows, &live, err)) return false;
    *out = idm_atom(rt, live ? "ok" : "closed");
    return true;
}
...
static bool prim_job_signal(IdmRuntime *rt, const IdmValue *args, IdmValue *out, IdmError *err) {
    ...
    int signo = job_arg_atom(args[1], "hup") ? SIGHUP
              ...
              : job_arg_atom(args[1], "stop") ? SIGTSTP
              : job_arg_atom(args[1], "term") ? SIGTERM
              : job_arg_atom(args[1], "kill") ? SIGKILL
              : job_arg_atom(args[1], "int") ? SIGINT
              : 0;
    if (signo == 0) return idm_primitive_type_error(rt, err, "job-signal", args[1], ":hup, :cont, :stop, :term, :kill, or :int");
...
bool idm_prim_invoke(IdmRuntime *rt, IdmPrimitive prim, const IdmValue *args, uint32_t argc, IdmValue *out, IdmError *err) {
    ...
    switch (prim) {
        ...
        case IDM_PRIM_SELECTOR_INFO: return prim_selector_info(rt, args, out, err);
        case IDM_PRIM_INSPECT: return prim_inspect(rt, args, out, err);
        case IDM_PRIM_JSON_STRING: return prim_json_string(rt, args, out, err);
        ...
        case IDM_PRIM_STR_FIND_RANGE: return prim_str_find_range(rt, args, out, err);
        case IDM_PRIM_STR_BYTE: return prim_str_byte(rt, args, out, err);
        case IDM_PRIM_BYTE_STR: return prim_byte_str(rt, args, out, err);
        case IDM_PRIM_STR_LINE_OF: return prim_str_line_of(rt, args, out, err);
        case IDM_PRIM_STR_LINE_START: return prim_str_line_start(rt, args, out, err);
        ...
        case IDM_PRIM_TOTAL_ALLOCS: return prim_total_allocs(rt, args, out, err);
        case IDM_PRIM_TOTAL_ALLOC_BYTES: return prim_total_alloc_bytes(rt, args, out, err);
        case IDM_PRIM_DICT_GET: return prim_dict_get(rt, args, out, err);
        case IDM_PRIM_DICT_PUT: return prim_dict_put(rt, args, out, err);
        case IDM_PRIM_RECORD_UPDATE: return prim_record_update(rt, args, out, err);
        case IDM_PRIM_HELP: return prim_help(rt, args, out, err);
        ...
        case IDM_PRIM_COMPILE: return prim_compile(rt, args, out, err);
        case IDM_PRIM_ABORT: return prim_abort(rt, args, out, err);
        case IDM_PRIM_SESSION: return prim_session(rt, out);
        ...
        case IDM_PRIM_PORT_RESIZE: return prim_port_resize(rt, args, out, err);
        case IDM_PRIM_JOB_SIGNAL: return prim_job_signal(rt, args, out, err);
        case IDM_PRIM_BITS_P: return prim_type_pred(rt, prim, args, out, err);
        case IDM_PRIM_BITS_LEN: return prim_bits_len(rt, args, out, err);
        case IDM_PRIM_BITS_SLICE: return prim_bits_slice(rt, args, out, err);
        case IDM_PRIM_BITS_INT: return prim_bits_int(rt, args, out, err);
        case IDM_PRIM_BITS_FLOAT: return prim_bits_float(rt, args, out, err);
        case IDM_PRIM_BITS_OF_INT: return prim_bits_of_int(rt, args, out, err);
        case IDM_PRIM_BITS_OF_FLOAT: return prim_bits_of_float(rt, args, out, err);
        case IDM_PRIM_BITS_APPEND: return prim_bits_append(rt, args, out, err);
        case IDM_PRIM_STRING_BITS: return prim_string_bits(rt, args, out, err);
        case IDM_PRIM_BITS_STRING: return prim_bits_string(rt, args, out, err);
        case IDM_PRIM_COUNT: break;
    }
