...
static void idm_profile_report(FILE *out);
...
static char *root_derived;

static void idm_root_derive(void) {
    char path[4096];
    ...
    for (;;) {
        ...
        *slash = '\0';
        char probe[4352];
        int len = snprintf(probe, sizeof(probe), "%s/std/kernel/bootstrap.id", path);
        if (len > 0 && (size_t)len < sizeof(probe) && access(probe, R_OK) == 0) {
            root_derived = idm_strdup(path);
            return;
        }
    }
}
...
const char *idm_root(void) {
    ...
    pthread_once(&root_once, idm_root_derive);
    return root_derived;
}
...
IdmProfileMetricHandle idm_profile_metric_handle(const char *name) {
    if (!idm_profile_enabled() || !name || !*name) return 0;
    pthread_mutex_lock(&profile_lock);
    IdmProfileMetric *m = profile_metric_get(name);
    IdmProfileMetricHandle handle = 0;
    if (m) {
        size_t index = (size_t)(m - profile_metrics);
        if (index < UINT32_MAX) handle = (IdmProfileMetricHandle)(index + 1u);
    }
...
void idm_profile_count_handle(IdmProfileMetricHandle handle, uint64_t amount) {
    if (!idm_profile_enabled() || handle == 0) return;
    size_t index = (size_t)handle - 1u;
    pthread_mutex_lock(&profile_lock);
    if (index < profile_metric_count) profile_metrics[index].count += amount;
    pthread_mutex_unlock(&profile_lock);
}
...
static IdmSpan span_own(IdmSpan span) {
    span.file = span.file ? idm_strdup(span.file) : NULL;
    return span;
}

void idm_error_set_span(IdmError *err, IdmSpan span) {
    if (!err) return;
    if (err->span.file) free((char *)err->span.file);
    err->span = span_own(span);
}
...
void idm_error_notes_free(IdmErrorNote *notes, size_t count) {
    for (size_t i = 0; i < count; i++) {
        free(notes[i].message);
        if (notes[i].span.file) free((char *)notes[i].span.file);
    }
    free(notes);
}
...
void idm_error_clear(IdmError *err) {
    if (!err) return;
    free(err->message);
    idm_error_notes_free(err->notes, err->note_count);
    if (err->span.file) free((char *)err->span.file);
    free(err->reason);
    err->present = false;
    err->span = idm_span_unknown(NULL);
...
static bool error_note_at_v(IdmError *err, IdmSpan span, const char *fmt, va_list ap) {
    if (!err || !err->present) return false;
    char line[512];
    vsnprintf(line, sizeof(line), fmt, ap);
    if (err->note_count == err->note_cap &&
        !idm_grow((void **)&err->notes, &err->note_cap, sizeof(*err->notes), 4u, err->note_count + 1u)) return false;
    char *message = idm_strdup(line);
    if (!message) return false;
    err->notes[err->note_count].message = message;
    err->notes[err->note_count].span = span_own(span);
    err->note_count++;
    return false;
}
...
bool idm_error_note(IdmError *err, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    bool result = error_note_at_v(err, idm_span_unknown(NULL), fmt, ap);
    va_end(ap);
    return result;
}
...
bool idm_error_note_at(IdmError *err, IdmSpan span, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    bool result = error_note_at_v(err, span, fmt, ap);
    va_end(ap);
    return result;
}

static void error_excerpt_fprint(FILE *out, IdmSpan span) {
    if (span.line == 0 || !span.file || span.end < span.start) return;
    char *source = NULL;
    size_t len = 0;
    IdmError ignore;
    idm_error_init(&ignore);
    bool ok = idm_read_file(span.file, &source, &len, &ignore);
    idm_error_clear(&ignore);
    if (!ok) return;
    size_t line_start = 0;
    unsigned line = 1;
    while (line < span.line && line_start < len) {
    ...
    size_t caret_col = span.column != 0 ? (size_t)span.column - 1u : 0u;
    if (line_start + caret_col > line_end) {
        free(source);
        return;
    }
    size_t caret_count = 1;
    if (span.end > span.start && span.start == line_start + caret_col) {
        caret_count = span.end - span.start;
        if (line_start + caret_col + caret_count > line_end) caret_count = line_end - line_start - caret_col;
        if (caret_count == 0) caret_count = 1;
    }
    fprintf(out, "  %5u | %.*s\n", span.line, (int)(line_end - line_start), source + line_start);
    fprintf(out, "        | ");
    for (size_t i = line_start; i < line_start + caret_col; i++) fputc(source[i] == '\t' ? '\t' : ' ', out);
    for (size_t i = 0; i < caret_count; i++) fputc('^', out);
    fputc('\n', out);
    free(source);
}

static void error_note_fprint(FILE *out, const IdmErrorNote *note) {
    const char *message = note->message ? note->message : "";
    if (note->span.line != 0 && note->span.file) {
        fprintf(out, "  %s (%s:%u:%u)\n", message, note->span.file, note->span.line, note->span.column);
    } else {
        fprintf(out, "  %s\n", message);
    }
}
...
void idm_error_fprint(FILE *out, const IdmError *err) {
    ...
    error_excerpt_fprint(out, err->span);
    for (size_t i = 0; i < err->note_count; i++) error_note_fprint(out, &err->notes[i]);
}
...
bool idm_error_render(const IdmError *err, IdmBuffer *out) {
    ...
    bool ok = true;
    if (err->span.line != 0) {
        ok = idm_buf_appendf(out, "%s:%u:%u: error: %s", file, err->span.line, err->span.column, err->message ? err->message : "error");
    } else {
        ok = idm_buf_appendf(out, "%s: error: %s", file, err->message ? err->message : "error");
    }
    for (size_t i = 0; ok && i < err->note_count; i++) {
        const IdmErrorNote *note = &err->notes[i];
        const char *message = note->message ? note->message : "";
        if (note->span.line != 0 && note->span.file) {
            ok = idm_buf_appendf(out, "\n  %s (%s:%u:%u)", message, note->span.file, note->span.line, note->span.column);
        } else {
            ok = idm_buf_appendf(out, "\n  %s", message);
        }
    }
...
bool idm_buf_append_json_string(IdmBuffer *buf, const char *text, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        switch (ch) {
            ...
            default:
                if (ch < 32u) {
                    if (!idm_buf_appendf(buf, "\\u%04x", ch)) return false;
                } else if (!idm_buf_append_char(buf, (char)ch)) return false;
        }
    }
    return idm_buf_append_char(buf, '"');
...
bool idm_wire_open(IdmByteReader *r, const unsigned char *data, size_t len, uint32_t *out_section_count, IdmError *err) {
    ...
    if (version != idm_wire_version()) {
        return idm_error_set(err, idm_span_unknown(NULL), "wire format %u unsupported (this build writes %u)",
                             version, idm_wire_version());
    }
