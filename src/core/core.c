...
IdmCore *idm_core_form_build(IdmSyntaxBuildKind kind, IdmCore *ctx, IdmCore *payload, IdmSpan span) {
    IdmCore *core = core_alloc(IDM_CORE_FORM_BUILD, span);
    if (!core) return NULL;
...
IdmCore *idm_core_match_bind(IdmCore *pattern_fn, IdmCore *value, IdmCore *body, uint32_t first_slot, uint32_t count, IdmSpan span) {
    IdmCore *core = core_alloc(IDM_CORE_MATCH_BIND, span);
    if (!core) return NULL;
    core->as.match_bind.pattern_fn = pattern_fn;
    core->as.match_bind.value = value;
    core->as.match_bind.body = body;
    core->as.match_bind.first_slot = first_slot;
    core->as.match_bind.count = count;
    return core;
}
...
bool idm_core_fn_set_doc(IdmCore *core, const char *doc) {
    ...
    if (core->kind == IDM_CORE_FN_MULTI) {
        free(core->as.fn_multi.doc);
        core->as.fn_multi.doc = copy;
        return true;
    }
    free(copy);
    return true;
...
bool idm_core_dispatch_add_arg(IdmCore *core, IdmCore *arg) {
    if (!core || core->kind != IDM_CORE_DISPATCH || !arg) return false;
    if (core->as.dispatch.arg_count == core->as.dispatch.arg_cap &&
        !idm_grow((void **)&core->as.dispatch.args, &core->as.dispatch.arg_cap, sizeof(*core->as.dispatch.args), 4u, core->as.dispatch.arg_count + 1u)) return false;
    core->as.dispatch.args[core->as.dispatch.arg_count++] = arg;
    return true;
}
...
bool idm_core_dispatch_add_impl(IdmCore *core, uint32_t method, const char *type_identity, IdmArity arity, bool passthrough, uint32_t primitive, IdmCore *ref, uint8_t ref_state) {
    ...
    im->ref = ref;
    im->ref_state = ref_state;
    core->as.dispatch.impl_count++;
    return true;
}
...
bool idm_core_dispatch_add_field(IdmCore *core, IdmSymbol *type, IdmSymbol *field, const char *type_identity, uint32_t field_index, const IdmTypeTerm *contract) {
    ...
    if (contract) {
        if (!idm_type_term_copy(&f->contract, contract)) return false;
        f->has_contract = true;
    }
    core->as.dispatch.field_count++;
    return true;
}

void idm_core_dispatch_set_fallback(IdmCore *core, IdmCore *fallback, uint8_t fallback_state) {
    if (!core || core->kind != IDM_CORE_DISPATCH) return;
    core->as.dispatch.fallback = fallback;
    core->as.dispatch.fallback_state = fallback_state;
}
...
void idm_core_free(IdmCore *core) {
    ...
    switch (core->kind) {
        ...
        case IDM_CORE_MATCH_BIND:
            idm_core_free(core->as.match_bind.pattern_fn);
            idm_core_free(core->as.match_bind.value);
            idm_core_free(core->as.match_bind.body);
            break;
        case IDM_CORE_FN:
        ...
        case IDM_CORE_DISPATCH:
            ...
            idm_core_free(core->as.dispatch.fallback);
            break;
    }
    free(core);
}
...
static bool idm_primitive_total(IdmPrimitive prim);
...
bool idm_core_statement_discardable(const IdmCore *core) {
    ...
    switch (core->kind) {
        ...
        case IDM_CORE_PACKAGE_REF:
            return true;
        case IDM_CORE_RECORD_IS:
            return idm_core_statement_discardable(core->as.record_is.value);
        case IDM_CORE_RECORD_FIELD:
            return idm_core_statement_discardable(core->as.record_field.receiver);
        case IDM_CORE_LIST_CONS:
        case IDM_CORE_LIST_APPEND:
            return idm_core_statement_discardable(core->as.list_pair.head) && idm_core_statement_discardable(core->as.list_pair.tail);
        case IDM_CORE_VALUE_SEQUENCE:
            for (size_t i = 0; i < core->as.value_sequence.count; i++) {
                if (!idm_core_statement_discardable(core->as.value_sequence.items[i])) return false;
            }
            return true;
        case IDM_CORE_STRING_CONCAT:
            for (size_t i = 0; i < core->as.string_concat.count; i++) {
                if (!idm_core_statement_discardable(core->as.string_concat.items[i])) return false;
            }
            return true;
        case IDM_CORE_COND:
            return idm_core_statement_discardable(core->as.cond_expr.cond) &&
                   idm_core_statement_discardable(core->as.cond_expr.then_branch) &&
                   idm_core_statement_discardable(core->as.cond_expr.else_branch);
        case IDM_CORE_CALL: {
            ...
            for (size_t i = 0; i < core->as.call.arg_count; i++) {
                if (!idm_core_statement_discardable(core->as.call.args[i])) return false;
            }
            return true;
        }
        default:
...
typedef struct {
    ...
    LinkPatch *link_patches;
    size_t link_patch_count;
    size_t link_patch_cap;
} CompileModuleCtx;
    ...
    switch (core->kind) {
        case IDM_CORE_DISPATCH: {
            ...
            for (size_t i = 0; i < core->as.dispatch.arg_count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.dispatch.args[i]);
                if (child > max) max = child;
            }
            for (size_t i = 0; i < core->as.dispatch.method_count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.dispatch.methods[i].evidence);
                if (child > max) max = child;
            }
            for (size_t i = 0; i < core->as.dispatch.impl_count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.dispatch.impls[i].ref);
                if (child > max) max = child;
            }
            uint32_t fb = idm_core_max_local_plus_one(core->as.dispatch.fallback);
            if (fb > max) max = fb;
            return max;
        }
        case IDM_CORE_LOCAL_REF:
        ...
        case IDM_CORE_CAPTURE_REF:
            return 0;
        case IDM_CORE_CALL: {
            uint32_t max = idm_core_max_local_plus_one(core->as.call.callee);
            for (size_t i = 0; i < core->as.call.arg_count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.call.args[i]);
                if (child > max) max = child;
            }
            return max;
        }
        case IDM_CORE_LIST_CONS:
        case IDM_CORE_LIST_APPEND: {
            uint32_t max = idm_core_max_local_plus_one(core->as.list_pair.head);
            uint32_t tail_max = idm_core_max_local_plus_one(core->as.list_pair.tail);
        ...
        case IDM_CORE_VALUE_SEQUENCE: {
            uint32_t max = 0;
            for (size_t i = 0; i < core->as.value_sequence.count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.value_sequence.items[i]);
                if (child > max) max = child;
            }
            return max;
        }
        case IDM_CORE_FORM_BUILD: {
            uint32_t max = idm_core_max_local_plus_one(core->as.syntax_build.ctx);
            uint32_t payload_max = idm_core_max_local_plus_one(core->as.syntax_build.payload);
        ...
        case IDM_CORE_STRING_CONCAT: {
            uint32_t max = 0;
            for (size_t i = 0; i < core->as.string_concat.count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.string_concat.items[i]);
                if (child > max) max = child;
            }
            return max;
        }
        case IDM_CORE_COND: {
            uint32_t max = idm_core_max_local_plus_one(core->as.cond_expr.cond);
            uint32_t then_max = idm_core_max_local_plus_one(core->as.cond_expr.then_branch);
            uint32_t else_max = idm_core_max_local_plus_one(core->as.cond_expr.else_branch);
        ...
        case IDM_CORE_MATCH: {
            ...
            for (size_t i = 0; i < core->as.match_expr.scrutinee_count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.match_expr.scrutinees[i]);
                if (child > max) max = child;
            }
        ...
        case IDM_CORE_DO: {
            ...
            for (size_t i = 0; i < core->as.do_expr.count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.do_expr.items[i]);
                if (child > max) max = child;
            }
            return max;
        }
        case IDM_CORE_BIND_LOCAL: {
            ...
            uint32_t value_max = idm_core_max_local_plus_one(core->as.bind_local.value);
            uint32_t body_max = idm_core_max_local_plus_one(core->as.bind_local.body);
            if (value_max > max) max = value_max;
            if (body_max > max) max = body_max;
            return max;
        }
        case IDM_CORE_MATCH_BIND: {
            uint32_t max = core->as.match_bind.first_slot + core->as.match_bind.count;
            uint32_t value_max = idm_core_max_local_plus_one(core->as.match_bind.value);
            uint32_t body_max = idm_core_max_local_plus_one(core->as.match_bind.body);
            if (value_max > max) max = value_max;
            if (body_max > max) max = body_max;
            return max;
        }
        ...
        case IDM_CORE_LETREC: {
            uint32_t max = idm_core_max_local_plus_one(core->as.letrec.body);
            for (size_t i = 0; i < core->as.letrec.count; i++) {
                uint32_t value_max = idm_core_max_local_plus_one(core->as.letrec.bindings[i].value);
                if (value_max > max) max = value_max;
        ...
        case IDM_CORE_RECEIVE: {
            uint32_t max = idm_core_max_local_plus_one(core->as.receive.receiver);
            uint32_t t = idm_core_max_local_plus_one(core->as.receive.timeout);
            uint32_t b = idm_core_max_local_plus_one(core->as.receive.timeout_body);
            if (t > max) max = t;
            if (b > max) max = b;
            return max;
        }
        case IDM_CORE_GUARD: {
            uint32_t max = idm_core_max_local_plus_one(core->as.guard.body);
            if (core->as.guard.handler) {
                uint32_t h = idm_core_max_local_plus_one(core->as.guard.handler);
                uint32_t s = core->as.guard.rescue_slot + 1u;
                if (h > max) max = h;
                if (s > max) max = s;
            }
            if (core->as.guard.cleanup) {
                uint32_t c = idm_core_max_local_plus_one(core->as.guard.cleanup);
        ...
        case IDM_CORE_USE_PACKAGE:
            return idm_core_max_local_plus_one(core->as.use_package.cont);
        case IDM_CORE_RECORD_CONSTRUCT: {
            uint32_t max = 0;
            for (size_t i = 0; i < core->as.record_construct.count; i++) {
                uint32_t child = idm_core_max_local_plus_one(core->as.record_construct.field_values[i]);
                if (child > max) max = child;
            }
            return max;
        }
        case IDM_CORE_RECORD_FIELD:
            return idm_core_max_local_plus_one(core->as.record_field.receiver);
        case IDM_CORE_RECORD_IS:
            return idm_core_max_local_plus_one(core->as.record_is.value);
        case IDM_CORE_LITERAL:
        case IDM_CORE_ENV_REF:
        case IDM_CORE_PACKAGE_REF:
            return 0;
...
static void collect_celled_slots(IdmCore *core, bool *celled, uint32_t lc) {
    ...
    switch (core->kind) {
        case IDM_CORE_DISPATCH:
            for (size_t i = 0; i < core->as.dispatch.arg_count; i++) collect_celled_slots(core->as.dispatch.args[i], celled, lc);
            for (size_t i = 0; i < core->as.dispatch.method_count; i++) collect_celled_slots(core->as.dispatch.methods[i].evidence, celled, lc);
            for (size_t i = 0; i < core->as.dispatch.impl_count; i++) collect_celled_slots(core->as.dispatch.impls[i].ref, celled, lc);
            collect_celled_slots(core->as.dispatch.fallback, celled, lc);
            break;
        case IDM_CORE_CALL:
        ...
        case IDM_CORE_BIND_LOCAL:
            ...
            return;
        case IDM_CORE_MATCH_BIND:
            collect_celled_slots(core->as.match_bind.value, celled, lc);
            collect_celled_slots(core->as.match_bind.body, celled, lc);
            return;
        case IDM_CORE_FN:
        case IDM_CORE_FN_MULTI:
            return;
...
static void mark_celled(IdmCore *core, const bool *celled, uint32_t lc, const IdmCapture *caps, size_t ncaps) {
    ...
    switch (core->kind) {
        case IDM_CORE_DISPATCH:
            for (size_t i = 0; i < core->as.dispatch.arg_count; i++) mark_celled(core->as.dispatch.args[i], celled, lc, caps, ncaps);
            for (size_t i = 0; i < core->as.dispatch.method_count; i++) mark_celled(core->as.dispatch.methods[i].evidence, celled, lc, caps, ncaps);
            for (size_t i = 0; i < core->as.dispatch.impl_count; i++) mark_celled(core->as.dispatch.impls[i].ref, celled, lc, caps, ncaps);
            mark_celled(core->as.dispatch.fallback, celled, lc, caps, ncaps);
            break;
        case IDM_CORE_LOCAL_REF:
        ...
        case IDM_CORE_BIND_LOCAL:
            ...
            return;
        case IDM_CORE_MATCH_BIND:
            mark_celled(core->as.match_bind.value, celled, lc, caps, ncaps);
            mark_celled(core->as.match_bind.body, celled, lc, caps, ncaps);
            return;
        case IDM_CORE_FN:
...
const IdmPrimitiveInfo idm_primitive_infos[IDM_PRIM_COUNT] = {
...
bool idm_primitive_pure(IdmPrimitive primitive) {
    if ((size_t)primitive >= (size_t)IDM_PRIM_COUNT) return false;
    return idm_primitive_infos[primitive].pure;
}

static bool idm_primitive_total(IdmPrimitive primitive) {
    if ((size_t)primitive >= (size_t)IDM_PRIM_COUNT) return false;
    return idm_primitive_infos[primitive].total;
}

size_t idm_primitive_count(void) {
    return (size_t)IDM_PRIM_COUNT;
}
...
static void compile_module_ctx_destroy(CompileModuleCtx *ctx) {
    ...
    free(ctx->link_patches);
    memset(ctx, 0, sizeof(*ctx));
}

static bool compile_module_ctx_pending_package(CompileModuleCtx *ctx, IdmValue env_key, const IdmBytecodeModule *module, size_t *out_index, IdmError *err, IdmSpan span) {
    for (size_t i = 0; i < ctx->linked_package_count; i++) {
        if (idm_value_equal(ctx->linked_packages[i].env_key, env_key)) { *out_index = i; return true; }
    }
    ...
    ctx->linked_packages[ctx->linked_package_count].module = module;
    ctx->linked_packages[ctx->linked_package_count].fn_off = 0;
    *out_index = ctx->linked_package_count++;
    return true;
}

static bool compile_module_ctx_add_patch(CompileModuleCtx *ctx, size_t operand_offset, size_t package_index, uint32_t init_fn, IdmError *err, IdmSpan span) {
    if (ctx->link_patch_count == ctx->link_patch_cap) {
        if (!idm_grow((void **)&ctx->link_patches, &ctx->link_patch_cap, sizeof(*ctx->link_patches), 8u, ctx->link_patch_count + 1u)) return idm_error_oom(err, span);
    }
    ctx->link_patches[ctx->link_patch_count].operand_offset = operand_offset;
    ctx->link_patches[ctx->link_patch_count].package_index = package_index;
    ctx->link_patches[ctx->link_patch_count].init_fn = init_fn;
    ctx->link_patch_count++;
    return true;
}

static bool compile_module_ctx_resolve_links(CompileModuleCtx *ctx, IdmBytecodeModule *module, IdmError *err) {
    for (size_t i = 0; i < ctx->linked_package_count; i++) {
        uint32_t fn_off = 0;
        if (!idm_bc_link(module, ctx->linked_packages[i].module, NULL, &fn_off, NULL, err)) return false;
        ctx->linked_packages[i].fn_off = fn_off;
    }
    for (size_t i = 0; i < ctx->link_patch_count; i++) {
        const LinkPatch *patch = &ctx->link_patches[i];
        uint32_t target = ctx->linked_packages[patch->package_index].fn_off + patch->init_fn;
        if (patch->operand_offset >= module->code_count) return idm_error_set(err, idm_span_unknown(NULL), "package link patch out of bounds");
        if (!idm_bc_patch_u32(module, patch->operand_offset, target)) return idm_error_set(err, idm_span_unknown(NULL), "failed to patch package init closure");
    }
    return true;
}
...
static bool compile_callable_entries(IdmCore *fn, IdmBytecodeModule *module, CompiledCallable *out, IdmError *err, IdmSpan span) {
    ...
    if (fn->kind == IDM_CORE_FN) {
        ...
        uint32_t locals = max_u32(idm_core_max_local_plus_one(fn->as.fn.body), pattern_locals);
        if (!add_compiled_function(module, fn->as.fn.name, fn->as.fn.arity, locals, fn->as.fn.param_patterns, fn->as.fn.pattern_count, fn->as.fn.pattern_locals, fn->as.fn.pattern_local_count, &out->entries[0], err, span)) {
            (void)0;
        } else if (fn->as.fn.doc && !idm_bc_set_function_doc(module, out->entries[0], fn->as.fn.doc)) {
        ...
        if (fn->as.fn.guard) {
            uint32_t guard_locals = max_u32(idm_core_max_local_plus_one(fn->as.fn.guard), pattern_locals);
    ...
    for (size_t i = 0; i < fn->as.fn_multi.count; i++) {
        ...
        uint32_t locals = max_u32(idm_core_max_local_plus_one(clause->body), pattern_locals);
        if (!add_compiled_function(module, fn->as.fn_multi.name, clause->arity, locals, clause->param_patterns, clause->pattern_count, clause->pattern_locals, clause->pattern_local_count, &out->entries[i], err, span)) {
            (void)0;
        } else if (fn->as.fn_multi.doc && !idm_bc_set_function_doc(module, out->entries[i], fn->as.fn_multi.doc)) {
            idm_error_oom(err, span);
        }
        if (err && err->present) {
            compiled_callable_destroy(out);
        ...
        if (clause->guard) {
            uint32_t guard_locals = max_u32(idm_core_max_local_plus_one(clause->guard), pattern_locals);
...
static bool core_direct_primitive_callable(const IdmCore *core, uint32_t argc, IdmPrimitive *out) {
    ...
    if (!idm_arity_accepts(&clause->call_arity, argc)) return false;
...
static bool compile_match_bind(IdmCore *core, CompileCtx *ctx, uint32_t dst, bool tail, IdmError *err) {
    IdmCore *fn = core->as.match_bind.pattern_fn;
    CompiledCallable callable;
    if (!compile_callable_entries(fn, ctx->module, &callable, err, fn->span)) return false;
    uint32_t mark = ctx_mark(ctx);
    uint32_t scrutinee = ctx_alloc(ctx);
    bool ok = compile_expr(core->as.match_bind.value, ctx, scrutinee, false, err);
    if (ok) ok = emit_op4(ctx->module, IDM_OP_MATCH_BIND, callable.entries[0], scrutinee, ctx_local_reg(ctx, core->as.match_bind.first_slot), core->as.match_bind.count, NULL) || idm_error_oom(err, core->span);
    size_t jump_over_op = 0;
    if (ok && !emit_op1(ctx->module, IDM_OP_JUMP, 0, &jump_over_op)) ok = idm_error_oom(err, fn->span);
    if (ok) ok = compile_callable_bodies(ctx->module_ctx, fn, &callable, ctx->module, err, fn->span);
    if (ok) ok = patch_here(ctx->module, jump_over_op + 1u, err, fn->span, "match bind carrier jump");
    compiled_callable_destroy(&callable);
    ctx_reset(ctx, mark);
    if (!ok) return false;
    return compile_expr(core->as.match_bind.body, ctx, dst, tail, err);
}
...
static bool compile_call_expr(IdmCore *core, CompileCtx *ctx, uint32_t dst, bool tail, IdmError *err) {
    ...
    if (core_direct_primitive_callable(core->as.call.callee, argc, &primitive)) {
        uint32_t mark = ctx_mark(ctx);
...
static bool compile_expr(IdmCore *core, CompileCtx *ctx, uint32_t dst, bool tail, IdmError *err) {
    ...
    switch (core->kind) {
        case IDM_CORE_DISPATCH:
            return idm_error_set(err, core->span, "internal: unlowered dispatch core reached compile");
        case IDM_CORE_LITERAL: {
        ...
        case IDM_CORE_MATCH_BIND:
            return compile_match_bind(core, ctx, dst, tail, err);
        case IDM_CORE_FN: {
        ...
        case IDM_CORE_USE_PACKAGE: {
            ...
            if (core->as.use_package.module) {
                if (!ctx->module_ctx) return idm_error_set(err, core->span, "missing compiler module context");
                if (!compile_module_ctx_pending_package(ctx->module_ctx, core->as.use_package.env_key, core->as.use_package.module, &pkg_index, err, core->span)) return false;
            }
            ...
            if (core->as.use_package.module) {
                ...
                size_t make_op = 0;
                if (!emit_op2(ctx->module, IDM_OP_MAKE_CLOSURE, closure, 0u, &make_op)) return idm_error_oom(err, core->span);
                if (!compile_module_ctx_add_patch(ctx->module_ctx, make_op + 2u, pkg_index, core->as.use_package.init_fn, err, core->span)) return false;
...
bool idm_core_compile_expression(IdmRuntime *rt, IdmCore *core, IdmBytecodeModule *module, IdmError *err) {
    ...
    uint32_t fn = 0;
    uint32_t locals = idm_core_max_local_plus_one(core);
...
bool idm_core_compile_function_body(IdmRuntime *rt, IdmCore *body, const char *name, uint32_t arity, IdmBytecodeModule *module, uint32_t *out_function, IdmError *err) {
    ...
    uint32_t fn = 0;
    uint32_t locals = idm_core_max_local_plus_one(body);
...
bool idm_core_compile_main(IdmRuntime *rt, IdmCore *core, IdmBytecodeModule *module, uint32_t *out_function, IdmError *err) {
    ...
    if (ok) ok = compile_module_ctx_resolve_links(&module_ctx, module, err);
    compile_module_ctx_destroy(&module_ctx);
    if (!ok) {
        idm_profile_scope_end(&prof);
        return false;
...
bool idm_core_dump(IdmBuffer *buf, const IdmCore *core) {
    ...
    switch (core->kind) {
        case IDM_CORE_DISPATCH: {
            const char *kind = core->as.dispatch.kind == IDM_DISPATCH_METHOD ? "method"
                             : core->as.dispatch.kind == IDM_DISPATCH_FIELD ? "field"
                             : "implements";
            if (!idm_buf_appendf(buf, "(dispatch-%s %s", kind, core->as.dispatch.name ? core->as.dispatch.name : "?")) return false;
            for (size_t i = 0; i < core->as.dispatch.arg_count; i++) {
                if (!idm_buf_append_char(buf, ' ') || !idm_core_dump(buf, core->as.dispatch.args[i])) return false;
            }
        ...
        case IDM_CORE_BIND_LOCAL:
            return idm_buf_appendf(buf, "(bind-local %s#%u ", core->as.bind_local.name, core->as.bind_local.slot) &&
                   ...
                   idm_buf_append_char(buf, ')');
        case IDM_CORE_MATCH_BIND:
            return idm_buf_appendf(buf, "(match-bind #%u+%u ", core->as.match_bind.first_slot, core->as.match_bind.count) &&
                   idm_core_dump(buf, core->as.match_bind.pattern_fn) &&
                   idm_buf_append_char(buf, ' ') &&
                   idm_core_dump(buf, core->as.match_bind.value) &&
                   idm_buf_append_char(buf, ' ') &&
                   idm_core_dump(buf, core->as.match_bind.body) &&
                   idm_buf_append_char(buf, ')');
        case IDM_CORE_FN:
...
static bool core_pretty(IdmBuffer *buf, const IdmCore *core, size_t indent) {
    ...
    switch (core->kind) {
        ...
        case IDM_CORE_BIND_LOCAL:
            ...
            return idm_buf_append_char(buf, ')');
        case IDM_CORE_MATCH_BIND:
            if (!idm_buf_appendf(buf, "(match-bind #%u+%u", core->as.match_bind.first_slot, core->as.match_bind.count)) return false;
            if (!pretty_newline(buf, ci) || !core_pretty(buf, core->as.match_bind.pattern_fn, ci)) return false;
            if (!pretty_newline(buf, ci) || !core_pretty(buf, core->as.match_bind.value, ci)) return false;
            if (!pretty_newline(buf, ci) || !core_pretty(buf, core->as.match_bind.body, ci)) return false;
            return idm_buf_append_char(buf, ')');
        case IDM_CORE_FN:
            if (!idm_buf_appendf(buf, "(fn %s/%u", core->as.fn.name, core->as.fn.arity)) return false;
