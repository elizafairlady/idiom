...
static bool solve_class(IdmSubst *s, const IdmTypeCmp *cmp, const IdmConstraintSet *given,
    ...
    if (ty->kind == IDM_TYPE_UNION) {
        for (size_t i = 0; i < ty->arg_count; i++) {
            if (!solve_class(s, cmp, given, trait, &ty->args[i], oracle, oracle_user, residual, err, span)) return false;
        }
        return true;
    }
    IdmInstanceResult r = oracle ? oracle(oracle_user, trait, ty) : IDM_INST_UNKNOWN;
    if (r == IDM_INST_NO) {
        IdmBuffer tb;
        idm_buf_init(&tb);
