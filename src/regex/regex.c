...
static bool parse_escape_atom(RxParser *p, RxNode **out) {
    ...
    if ((c >= '1' && c <= '9') || c == 'k') return parser_error(p, "backreferences are not supported");
    switch (c) {
...
#define RX_COMPILE_NODE_BUDGET 100000u
...
static bool regex_size_error(IdmError *err) {
    return idm_error_set(err, idm_span_unknown(NULL), "regex size-budget exceeded (limit %u byte automaton nodes)", (unsigned)RX_COMPILE_NODE_BUDGET);
}

static bool rx_len_add(size_t a, size_t b, size_t *out) {
    if (a > SIZE_MAX - b) return false;
    *out = a + b;
    return true;
}

static bool rx_len_mul(size_t a, size_t b, size_t *out) {
    if (a != 0 && b > SIZE_MAX / a) return false;
    *out = a * b;
    return true;
}

static bool rx_match_len(const RxNode *node, RxMatchLen *out) {
    if (!node) {
        *out = (RxMatchLen){ .min = 0u, .max = 0u, .bounded = true };
        return true;
    }
    switch (node->kind) {
        case RX_EMPTY:
        ...
        case RX_DOT:
        case RX_CLASS:
            *out = (RxMatchLen){ .min = 1u, .max = 1u, .bounded = true };
            return true;
        case RX_CAPTURE:
            return rx_match_len(node->as.capture.child, out);
        case RX_CONCAT: {
            RxMatchLen total = { .min = 0u, .max = 0u, .bounded = true };
            for (size_t i = 0; i < node->as.seq.count; i++) {
                RxMatchLen child = {0};
                if (!rx_match_len(node->as.seq.items[i], &child)) return false;
                if (!rx_len_add(total.min, child.min, &total.min)) return false;
                if (!total.bounded || !child.bounded) {
                    total.bounded = false;
                    total.max = SIZE_MAX;
                } else if (!rx_len_add(total.max, child.max, &total.max)) {
                    return false;
                }
            }
            *out = total;
        ...
        case RX_ALT: {
            RxMatchLen total = { .min = SIZE_MAX, .max = 0u, .bounded = true };
            ...
            for (size_t i = 0; i < node->as.seq.count; i++) {
                RxMatchLen child = {0};
                if (!rx_match_len(node->as.seq.items[i], &child)) return false;
                if (child.min < total.min) total.min = child.min;
                if (!child.bounded) {
                    total.bounded = false;
                    total.max = SIZE_MAX;
                } else if (total.bounded && child.max > total.max) {
                    total.max = child.max;
                }
            }
            *out = total;
        ...
        case RX_REPEAT: {
            RxMatchLen child = {0};
            if (!rx_match_len(node->as.repeat.child, &child)) return false;
            RxMatchLen total = { .min = 0u, .max = 0u, .bounded = true };
            if (!rx_len_mul(child.min, node->as.repeat.min, &total.min)) return false;
            if (node->as.repeat.unbounded) {
                if (child.bounded && child.max == 0u) {
                    total.max = 0u;
                } else {
                    total.bounded = false;
                    total.max = SIZE_MAX;
            ...
            if (!rx_len_mul(child.max, node->as.repeat.max, &total.max)) return false;
            *out = total;
            return true;
        }
    }
...
static bool rx_size_add(size_t *total, size_t add) {
    if (add > RX_COMPILE_NODE_BUDGET || *total > RX_COMPILE_NODE_BUDGET - add) return false;
    *total += add;
    return true;
}

static bool rx_size_mul(size_t value, size_t count, size_t *out) {
    if (value != 0 && count > RX_COMPILE_NODE_BUDGET / value) return false;
    *out = value * count;
    return *out <= RX_COMPILE_NODE_BUDGET;
}

static bool rx_compiled_size(const RxNode *node, size_t *out) {
    if (!node) {
        *out = 0;
        return true;
    }
    switch (node->kind) {
        ...
        case RX_CONCAT: {
            ...
            for (size_t i = 0; i < node->as.seq.count; i++) {
                size_t child = 0;
                if (!rx_compiled_size(node->as.seq.items[i], &child) || !rx_size_add(&total, child)) return false;
            }
            *out = total;
            return true;
        }
        case RX_ALT: {
            size_t total = 0;
            if (node->as.seq.count > 1u && !rx_size_mul(2u, node->as.seq.count - 1u, &total)) return false;
            for (size_t i = 0; i < node->as.seq.count; i++) {
                size_t child = 0;
                if (!rx_compiled_size(node->as.seq.items[i], &child) || !rx_size_add(&total, child)) return false;
            }
        ...
        case RX_REPEAT: {
            size_t child = 0;
            if (!rx_compiled_size(node->as.repeat.child, &child)) return false;
            if (child == 0) {
                *out = 0;
                return true;
            }
            size_t total = 0;
            size_t fixed = 0;
            if (!rx_size_mul(child, node->as.repeat.min, &fixed) || !rx_size_add(&total, fixed)) return false;
            if (node->as.repeat.unbounded) {
                if (!rx_size_add(&total, child) || !rx_size_add(&total, 2u)) return false;
            } else {
                size_t optional_width = child + 1u;
                if (optional_width == 0 || optional_width > RX_COMPILE_NODE_BUDGET) return false;
                size_t optional = 0;
                if (!rx_size_mul(optional_width, node->as.repeat.max - node->as.repeat.min, &optional) || !rx_size_add(&total, optional)) return false;
            }
            *out = total;
            return true;
        }
        case RX_CAPTURE: {
            size_t child = 0;
            if (!rx_compiled_size(node->as.capture.child, &child)) return false;
            if (!rx_size_add(&child, 2u)) return false;
            *out = child;
            return true;
        }
        case RX_LOOK_POS:
        ...
        case RX_LOOKBEHIND_NEG: {
            size_t child = 0;
            if (!rx_compiled_size(node->as.child, &child)) return false;
            if (!rx_size_add(&child, 2u)) return false;
            *out = child;
            return true;
        }
    }
    *out = RX_COMPILE_NODE_BUDGET + 1u;
    return false;
}

static bool rx_check_compiled_size(const RxNode *root, size_t extra, IdmError *err) {
    size_t total = extra;
    size_t root_size = 0;
    if (total > RX_COMPILE_NODE_BUDGET || !rx_compiled_size(root, &root_size) || !rx_size_add(&total, root_size)) return regex_size_error(err);
    return true;
}

static bool rx_budget_node(RxCompiler *c, IdmError *err) {
    if (c->budget->used >= RX_COMPILE_NODE_BUDGET) return regex_size_error(err);
    c->budget->used++;
    return true;
}

static uint32_t rx_fork(RxCompiler *c, IdmError *err) {
    if (!rx_budget_node(c, err)) return SEL_NO_NODE;
    return idm_byteprog_fork(c->bp, err);
}

static uint32_t rx_byte(RxCompiler *c, const IdmByteClass *cls, IdmError *err) {
    if (!rx_budget_node(c, err)) return SEL_NO_NODE;
    return idm_byteprog_byte(c->bp, cls, err);
}

static uint32_t rx_save(RxCompiler *c, uint32_t slot, IdmError *err) {
    if (!rx_budget_node(c, err)) return SEL_NO_NODE;
    return idm_byteprog_save(c->bp, slot, err);
}

static uint32_t rx_guard(RxCompiler *c, IdmByteGuardKind kind, SelByteProg *sub, IdmError *err) {
    if (!rx_budget_node(c, err)) {
        idm_byteprog_free(sub);
        return SEL_NO_NODE;
    }
    return idm_byteprog_guard(c->bp, kind, c->flags, sub, err);
}

static uint32_t rx_lookbehind_guard(RxCompiler *c, IdmByteGuardKind kind, SelByteProg *sub, size_t min_len, size_t max_len, IdmError *err) {
    if (!rx_budget_node(c, err)) {
        idm_byteprog_free(sub);
        return SEL_NO_NODE;
    }
    return idm_byteprog_lookbehind_guard(c->bp, kind, c->flags, sub, min_len, max_len, err);
}

static uint32_t rx_accept(RxCompiler *c, uint32_t accept_id, IdmError *err) {
    if (!rx_budget_node(c, err)) return SEL_NO_NODE;
    return idm_byteprog_accept(c->bp, accept_id, err);
}

static void start_info_add_byte(IdmByteStartSet *info, unsigned char ch, uint32_t flags) {
    info->bytes[ch] = true;
...
static void start_info_add_class(IdmByteStartSet *info, const IdmByteClass *cls) {
    for (unsigned i = 0; i < 256u; i++) {
        if (idm_byteclass_has(cls, (unsigned char)i)) info->bytes[i] = true;
    }
}

static void start_info_union(IdmByteStartSet *dst, const IdmByteStartSet *src) {
    dst->any = dst->any || src->any;
    dst->nullable = dst->nullable || src->nullable;
    for (size_t i = 0; i < 256u; i++) dst->bytes[i] = dst->bytes[i] || src->bytes[i];
}

static IdmByteStartSet start_info_node(const RxNode *node, uint32_t flags) {
    IdmByteStartSet info = {0};
    if (!node) {
        info.nullable = true;
        return info;
    }
    switch (node->kind) {
        ...
        case RX_ALT:
            ...
            for (size_t i = 0; i < node->as.seq.count; i++) {
                IdmByteStartSet child = start_info_node(node->as.seq.items[i], flags);
...
static bool bcompile_node(RxCompiler *c, const RxNode *node, IdmError *err);

static bool bcompile_subprog(RxCompiler *c, const RxNode *node, SelByteProg **out, IdmError *err) {
    *out = idm_byteprog_new(err);
    if (!*out) return false;
    idm_byteprog_set_flags(*out, c->flags);
    idm_byteprog_set_capture_count(*out, 0);
    RxCompiler sub = { .bp = *out, .flags = c->flags, .budget = c->budget };
    if (!bcompile_node(&sub, node, err) || rx_accept(&sub, 0, err) == SEL_NO_NODE) {
...
static bool bcompile_alt_from(RxCompiler *c, RxNode **items, size_t index, size_t count, IdmError *err) {
    if (index >= count) return true;
    uint32_t *jumps = NULL;
    size_t jc = 0, jcap = 0;
    bool ok = true;
    for (size_t i = index; ok && i + 1u < count; i++) {
        uint32_t split = rx_fork(c, err);
        if (split == SEL_NO_NODE) { ok = false; break; }
        uint32_t first = (uint32_t)idm_byteprog_node_count(c->bp);
        if (!bcompile_node(c, items[i], err)) { ok = false; break; }
        uint32_t jump = rx_fork(c, err);
        if (jump == SEL_NO_NODE) { ok = false; break; }
        uint32_t second = (uint32_t)idm_byteprog_node_count(c->bp);
        idm_byteprog_set_fork(c->bp, split, first, second);
        if (jc == jcap) {
            if (!idm_grow((void **)&jumps, &jcap, sizeof(*jumps), 8u, jc + 1u)) { idm_error_oom(err, idm_span_unknown(NULL)); ok = false; break; }
        }
        jumps[jc++] = jump;
    }
    if (ok) ok = bcompile_node(c, items[count - 1u], err);
    uint32_t end = (uint32_t)idm_byteprog_node_count(c->bp);
    for (size_t i = 0; ok && i < jc; i++) idm_byteprog_set_fork(c->bp, jumps[i], end, end);
    free(jumps);
    return ok;
}

static bool bcompile_star(RxCompiler *c, const RxNode *child, IdmError *err) {
    uint32_t split = rx_fork(c, err);
    if (split == SEL_NO_NODE) return false;
    uint32_t body = (uint32_t)idm_byteprog_node_count(c->bp);
    if (!bcompile_node(c, child, err)) return false;
    uint32_t jump = rx_fork(c, err);
    if (jump == SEL_NO_NODE) return false;
    idm_byteprog_set_fork(c->bp, jump, split, split);
    uint32_t after = (uint32_t)idm_byteprog_node_count(c->bp);
    idm_byteprog_set_fork(c->bp, split, body, after);
    return true;
}

static bool bcompile_optional(RxCompiler *c, const RxNode *child, IdmError *err) {
    uint32_t split = rx_fork(c, err);
    if (split == SEL_NO_NODE) return false;
    uint32_t body = (uint32_t)idm_byteprog_node_count(c->bp);
    if (!bcompile_node(c, child, err)) return false;
...
static bool bcompile_repeat(RxCompiler *c, const RxNode *child, size_t min, size_t max, bool unbounded, IdmError *err) {
    size_t child_size = 0;
    if (!rx_compiled_size(child, &child_size)) return regex_size_error(err);
    if (child_size == 0) return true;
    for (size_t i = 0; i < min; i++) {
        if (!bcompile_node(c, child, err)) return false;
    }
    if (unbounded) return bcompile_star(c, child, err);
    for (size_t i = min; i < max; i++) {
        if (!bcompile_optional(c, child, err)) return false;
    }
    return true;
}
...
static bool bcompile_node(RxCompiler *c, const RxNode *node, IdmError *err) {
    ...
    switch (node->kind) {
        ...
        case RX_LITERAL: {
            IdmByteClass cls = {0};
            idm_byteclass_add_char(&cls, node->as.literal, (c->flags & IDM_REGEX_CASELESS) != 0);
            return rx_byte(c, &cls, err) != SEL_NO_NODE;
        }
        case RX_DOT: {
            IdmByteClass cls = {0};
            for (unsigned ch = 0; ch < 256u; ch++) {
                if ((c->flags & IDM_REGEX_DOTALL) != 0 || ch != (unsigned)'\n') idm_byteclass_set(&cls, (unsigned char)ch);
            }
            return rx_byte(c, &cls, err) != SEL_NO_NODE;
        }
        case RX_CLASS:
            return rx_byte(c, &node->as.cls, err) != SEL_NO_NODE;
        case RX_ANCHOR_START:
            return rx_guard(c, IDM_BYTE_GUARD_LINE_START, NULL, err) != SEL_NO_NODE;
        case RX_ANCHOR_END:
            return rx_guard(c, IDM_BYTE_GUARD_LINE_END, NULL, err) != SEL_NO_NODE;
        case RX_WORD_BOUNDARY:
            return rx_guard(c, IDM_BYTE_GUARD_WORD_BOUNDARY, NULL, err) != SEL_NO_NODE;
        case RX_NOT_WORD_BOUNDARY:
            return rx_guard(c, IDM_BYTE_GUARD_NOT_WORD_BOUNDARY, NULL, err) != SEL_NO_NODE;
        case RX_BUFFER_START:
            return rx_guard(c, IDM_BYTE_GUARD_BUFFER_START, NULL, err) != SEL_NO_NODE;
        case RX_BUFFER_END:
            return rx_guard(c, IDM_BYTE_GUARD_BUFFER_END, NULL, err) != SEL_NO_NODE;
        case RX_BUFFER_END_NL:
            return rx_guard(c, IDM_BYTE_GUARD_BUFFER_END_NL, NULL, err) != SEL_NO_NODE;
        case RX_CONCAT:
            for (size_t i = 0; i < node->as.seq.count; i++) {
                if (!bcompile_node(c, node->as.seq.items[i], err)) return false;
            }
            return true;
        case RX_ALT:
            return bcompile_alt_from(c, node->as.seq.items, 0, node->as.seq.count, err);
        case RX_REPEAT:
            return bcompile_repeat(c, node->as.repeat.child, node->as.repeat.min, node->as.repeat.max, node->as.repeat.unbounded, err);
        case RX_CAPTURE: {
            if (rx_save(c, (uint32_t)(node->as.capture.index * 2u), err) == SEL_NO_NODE) return false;
            if (!bcompile_node(c, node->as.capture.child, err)) return false;
            return rx_save(c, (uint32_t)(node->as.capture.index * 2u + 1u), err) != SEL_NO_NODE;
        }
        case RX_LOOK_POS:
        case RX_LOOK_NEG:
        case RX_LOOKBEHIND_POS:
        case RX_LOOKBEHIND_NEG: {
            RxMatchLen width = {0};
            bool lookbehind = node->kind == RX_LOOKBEHIND_POS || node->kind == RX_LOOKBEHIND_NEG;
            if (lookbehind) {
                if (!rx_match_len(node->as.child, &width)) return regex_size_error(err);
                if (!width.bounded) return idm_error_set(err, idm_span_unknown(NULL), "regex lookbehind requires bounded length");
            }
            SelByteProg *sub = NULL;
            if (!bcompile_subprog(c, node->as.child, &sub, err)) return false;
            ...
            if (lookbehind) return rx_lookbehind_guard(c, gk, sub, width.min, width.max, err) != SEL_NO_NODE;
            return rx_guard(c, gk, sub, err) != SEL_NO_NODE;
        }
    }
    return true;
}
...
static bool build_byteprog(const RxNode *root, uint32_t flags, size_t capture_count, SelByteProg **out, IdmError *err) {
    if (!rx_check_compiled_size(root, 1u, err)) return false;
    *out = idm_byteprog_new(err);
    if (!*out) return false;
    idm_byteprog_set_flags(*out, flags);
    idm_byteprog_set_capture_count(*out, capture_count);
    RxCompileBudget budget = {0};
    RxCompiler compiler = { .bp = *out, .flags = flags, .budget = &budget };
    if (!bcompile_node(&compiler, root, err) || rx_accept(&compiler, 0, err) == SEL_NO_NODE) {
        idm_byteprog_free(*out);
        *out = NULL;
        return false;
    }
...
static bool build_byteprog_set(const IdmRegex *const *items, size_t count, size_t capture_count, SelByteProg **out, IdmError *err) {
    ...
    size_t total = 0;
    if (!rx_size_add(&total, count) || !rx_size_add(&total, split_count)) return regex_size_error(err);
    for (size_t i = 0; i < count; i++) {
        size_t item_size = 0;
        if (!rx_compiled_size(items[i]->root, &item_size) || !rx_size_add(&total, item_size)) return regex_size_error(err);
    }
    *out = idm_byteprog_new(err);
    if (!*out) return false;
    SelByteProg *bp = *out;
    ...
    bool ok = true;
    RxCompileBudget budget = {0};
    RxCompiler compiler = { .bp = bp, .flags = 0, .budget = &budget };
    for (size_t i = 0; ok && i < split_count; i++) {
        forks[i] = rx_fork(&compiler, err);
        if (forks[i] == SEL_NO_NODE) ok = false;
    }
    for (size_t i = 0; ok && i < count; i++) {
        starts[i] = (uint32_t)idm_byteprog_node_count(bp);
        compiler.flags = items[i]->flags;
        if (!bcompile_node(&compiler, items[i]->root, err) || rx_accept(&compiler, (uint32_t)i, err) == SEL_NO_NODE) ok = false;
    }
    for (size_t i = 0; ok && i < split_count; i++) {
...
static bool rx_search_test(const SelByteProg *bp, const IdmByteStartSet *start, IdmText *t, size_t offset, bool *out_matched, IdmError *err) {
    *out_matched = false;
    size_t scan_off = offset > t->len ? t->len : offset;
    IdmByteMatch m = {0};
    if (!idm_byteprog_search_text(bp, t, scan_off, start, false, &m, err)) return false;
...
static bool result_set_subject(IdmRegexResult *result, IdmValue subject, const char *s, size_t len, IdmError *err) {
    ...
    if (idm_value_tag(subject) == IDM_VAL_STRING && idm_string_length(subject) == len) {
...
static bool result_set_subject_text(IdmRegexResult *result, IdmValue subject, IdmText *t, IdmError *err) {
    result->subject_len = t->len;
    if (idm_value_tag(subject) == IDM_VAL_STRING && idm_string_length(subject) == t->len) {
        result->subject_value = subject;
        result->subject = NULL;
        result->owns_subject = false;
        return true;
    }
    result->subject = idm_strndup(t->flat ? t->flat : "", t->flat ? t->len : 0u);
    if (!result->subject) return idm_error_oom(err, idm_span_unknown(NULL));
    result->subject_value = idm_nil();
    result->owns_subject = true;
    return true;
...
static IdmRegexResult *result_new(const IdmRegex *rx, IdmValue subject, IdmText *t, size_t start, const IdmByteMatch *match, IdmError *err) {
    size_t capture_count = rx->group_count + 1u;
    IdmRegexResult *result = result_alloc_inline(capture_count, err);
    if (!result) return NULL;
    if (!result_set_subject_text(result, subject, t, err)) {
...
static bool scan_at_raw(const IdmRegex *rx, IdmValue subject, IdmText *t, size_t offset, bool full, IdmRegexResult **out, IdmError *err) {
    *out = NULL;
    if (offset > t->len) return true;
    IdmByteMatch match = {0};
    bool ok = idm_byteprog_match_text(rx->byteprog, t, offset, full, t->len, true, &match, err);
    if (!ok) return false;
    if (match.matched) *out = result_new(rx, subject, t, offset, &match, err);
    idm_byte_match_destroy(&match);
    return !(err && err->present);
}

static bool scan_from_raw(const IdmRegex *rx, IdmValue subject, IdmText *t, size_t offset, IdmRegexResult **out, IdmError *err) {
    *out = NULL;
    if (offset > t->len) offset = t->len;
    IdmByteMatch match = {0};
    if (!idm_byteprog_search_text(rx->byteprog, t, offset, &rx->start, true, &match, err)) return false;
    if (match.matched) *out = result_new(rx, subject, t, match.start, &match, err);
    idm_byte_match_destroy(&match);
    return !(err && err->present);
}
...
bool idm_regex_test_bytes(const IdmRegex *rx, const char *input, size_t input_len, bool *out_matched, IdmError *err) {
    ...
    IdmText t = idm_text_flat(input, input_len);
    return rx_search_test(rx->byteprog, &rx->start, &t, 0, out_matched, err);
}
...
bool idm_regex_exec_at_subject(const IdmRegex *rx, IdmValue subject, const char *input, size_t input_len, size_t offset, bool full, IdmRegexResult **out, IdmError *err) {
    ...
    IdmText t = idm_text_flat(input, input_len);
    return scan_at_raw(rx, subject, &t, offset, full, out, err);
}
...
bool idm_regex_scan_subject(const IdmRegex *rx, IdmValue subject, const char *input, size_t input_len, size_t offset, IdmRegexResult **out, IdmError *err) {
    ...
    IdmText t = idm_text_flat(input, input_len);
    return scan_from_raw(rx, subject, &t, offset, out, err);
}
...
static bool regex_and_text(IdmRuntime *rt, const char *name, IdmValue regex, IdmValue input, IdmRegex **out_rx, IdmText *t, IdmTextChunk **owned, IdmError *err) {
    *out_rx = idm_regex_value_get(regex, err);
    if (!*out_rx) return false;
    if (idm_value_tag(input) != IDM_VAL_STRING) return idm_primitive_type_error(rt, err, name, input, "a string");
    return idm_text_from_string(input, t, owned, err);
}
...
const SelByteProg *idm_regex_byteprog(const IdmRegex *rx) {
    return rx ? rx->byteprog : NULL;
}
...
IdmRegexResult *idm_regex_result_clone_with_subject(const IdmRegexResult *src, IdmValue subject, IdmError *err) {
    ...
    if (idm_value_tag(subject) == IDM_VAL_STRING) {
        ...
        subject_bytes = subject_len == src->subject_len ? NULL : idm_string_bytes(subject);
    }
...
bool idm_regex_scan_at(IdmRuntime *rt, IdmValue regex, IdmValue input, size_t offset, IdmValue *out, IdmError *err) {
    IdmRegex *rx = NULL;
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-scan-at", regex, input, &rx, &t, &owned, err)) return false;
    IdmRegexResult *result = NULL;
    bool ok = scan_at_raw(rx, input, &t, offset, false, &result, err);
    free(owned);
    if (!ok) return false;
...
bool idm_regex_scan_from(IdmRuntime *rt, IdmValue regex, IdmValue input, size_t offset, IdmValue *out, IdmError *err) {
    ...
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-scan-from", regex, input, &rx, &t, &owned, err)) return false;
    IdmRegexResult *result = NULL;
    bool ok = scan_from_raw(rx, input, &t, offset, &result, err);
    free(owned);
    if (!ok) return false;
    return wrap_result_or_nil(rt, result, out, err);
}
...
bool idm_regex_scan_full(IdmRuntime *rt, IdmValue regex, IdmValue input, IdmValue *out, IdmError *err) {
    ...
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-scan-full", regex, input, &rx, &t, &owned, err)) return false;
    IdmRegexResult *result = NULL;
    bool ok = scan_at_raw(rx, input, &t, 0, true, &result, err);
    free(owned);
    if (!ok) return false;
    return wrap_result_or_nil(rt, result, out, err);
}
...
bool idm_regex_test(IdmRuntime *rt, IdmValue regex, IdmValue input, IdmValue *out, IdmError *err) {
    ...
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-test?", regex, input, &rx, &t, &owned, err)) return false;
    bool matched = false;
    bool ok = rx_search_test(rx->byteprog, &rx->start, &t, 0, &matched, err);
    free(owned);
    if (!ok) return false;
...
bool idm_regex_scan_all(IdmRuntime *rt, IdmValue regex, IdmValue input, IdmValue *out, IdmError *err) {
    ...
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-scan-all", regex, input, &rx, &t, &owned, err)) return false;
    size_t len = t.len;
    IdmValue acc = idm_empty_list();
    size_t pos = 0;
    while (pos <= len) {
        IdmRegexResult *result = NULL;
        if (!scan_from_raw(rx, input, &t, pos, &result, err)) { free(owned); return false; }
        if (!result) break;
        ...
        if (err && err->present) { free(owned); return false; }
...
static IdmValue result_subject_slice(IdmRuntime *rt, const IdmRegexResult *r, size_t start, size_t end, IdmError *err) {
    if (r->subject) return idm_string_n(rt, r->subject + start, end - start, err);
    if (idm_value_tag(r->subject_value) == IDM_VAL_STRING) return idm_string_slice_value(rt, r->subject_value, start, end, err);
    return idm_string_n(rt, "", 0u, err);
}
...
bool idm_regex_result_text_value(IdmRuntime *rt, IdmValue result, IdmValue *out, IdmError *err) {
    ...
    *out = result_subject_slice(rt, r, c.start, c.end, err);
    return !(err && err->present);
}
...
static bool append_capture(IdmBuffer *buf, IdmRegexResult *r, size_t index) {
    ...
    IdmByteCapture c = r->captures[index];
    if (r->subject) return idm_buf_append_n(buf, r->subject + c.start, c.end - c.start);
    return idm_string_append_range(buf, r->subject_value, c.start, c.end);
}
...
bool idm_regex_replace(IdmRuntime *rt, IdmValue regex, IdmValue input, IdmValue replacement, bool all, IdmValue *out, IdmError *err) {
    ...
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-replace", regex, input, &rx, &t, &owned, err)) return false;
    size_t len = t.len;
    const char *repl = NULL;
    size_t repl_len = 0;
    if (!idm_primitive_require_string_arg(rt, replacement, &repl, &repl_len, "raw-replace", err)) { free(owned); return false; }
    IdmBuffer buf;
    idm_buf_init(&buf);
    size_t pos = 0;
    bool changed = false;
    while (pos <= len) {
        IdmRegexResult *result = NULL;
        if (!scan_from_raw(rx, input, &t, pos, &result, err)) {
            idm_buf_destroy(&buf);
            free(owned);
            return false;
        }
        if (!result) {
            if (changed && !idm_string_append_range(&buf, input, pos, len)) { idm_buf_destroy(&buf); free(owned); return idm_error_oom(err, idm_span_unknown(NULL)); }
            break;
        }
        IdmByteCapture whole = result->captures[0];
        if (!idm_string_append_range(&buf, input, pos, whole.start) || !append_replacement(&buf, result, repl, repl_len)) {
            idm_regex_result_free(result);
            idm_buf_destroy(&buf);
            free(owned);
            return idm_error_oom(err, idm_span_unknown(NULL));
        ...
        idm_regex_result_free(result);
        if (!all) {
            if (!idm_string_append_range(&buf, input, next, len)) { idm_buf_destroy(&buf); free(owned); return idm_error_oom(err, idm_span_unknown(NULL)); }
            pos = len + 1u;
            break;
        }
        if (next <= whole.start) {
            if (next < len && !idm_buf_append_char(&buf, (char)idm_text_byte(&t, next))) { idm_buf_destroy(&buf); free(owned); return idm_error_oom(err, idm_span_unknown(NULL)); }
...
bool idm_regex_split_on(IdmRuntime *rt, IdmValue regex, IdmValue input, IdmValue *out, IdmError *err) {
    ...
    IdmText t;
    IdmTextChunk *owned = NULL;
    if (!regex_and_text(rt, "raw-split-on", regex, input, &rx, &t, &owned, err)) return false;
    size_t len = t.len;
    IdmValue acc = idm_empty_list();
    ...
    while (scan_pos <= len) {
        IdmRegexResult *result = NULL;
        if (!scan_from_raw(rx, input, &t, scan_pos, &result, err)) { free(owned); return false; }
        if (!result) {
            IdmValue tail = idm_string_slice_value(rt, input, segment_start, len, err);
            if (err && err->present) { free(owned); return false; }
        ...
        IdmByteCapture whole = result->captures[0];
        IdmValue part = idm_string_slice_value(rt, input, segment_start, whole.start, err);
        if (err && err->present) { idm_regex_result_free(result); free(owned); return false; }
