...
static const char *quote_symbol_for_head(uint8_t form_id);
...
IdmCore *expand_string_literal(ExpandContext *ctx, const char *text, IdmSpan span, IdmError *err) {
    return string_literal(ctx, text, span, err);
}
...
static IdmCore *syntax_build_core(ExpandContext *ctx, IdmSyntaxBuildKind kind, IdmCore *payload, const IdmSyntax *template_ctx, IdmSpan span, IdmError *err) {
    ...
    IdmCore *core = idm_core_form_build(kind, context, payload, span);
    if (!core) {
...
static bool quasisyntax_splice_item(const IdmSyntax *item, const IdmSyntax **out_splice) {
    ...
    if (idm_syn_is_form_id(item, IDM_FORM_EXPR) && item->as.seq.count == 2 && idm_syn_is_form_id(item->as.seq.items[1], IDM_FORM_UNSYNTAX_SPLICING)) splice_item = item->as.seq.items[1];
    if (!idm_syn_is_form_id(splice_item, IDM_FORM_UNSYNTAX_SPLICING)) return false;
    *out_splice = splice_item;
    return true;
}

static bool quasiquote_splice_item(const IdmSyntax *item, const IdmSyntax **out_splice) {
    if (!idm_syn_is_form_id(item, IDM_FORM_UNQUOTE_SPLICING)) return false;
    *out_splice = item;
    return true;
}
...
static const char *quote_symbol_for_head(uint8_t form_id) {
    if (form_id == IDM_FORM_QUOTE) return "quote";
    if (form_id == IDM_FORM_QUASIQUOTE) return "quasiquote";
    if (form_id == IDM_FORM_UNQUOTE) return "unquote";
    if (form_id == IDM_FORM_UNQUOTE_SPLICING) return "unquote-splicing";
    return NULL;
}
...
static bool template_walk(ExpandContext *ctx, const IdmSyntax *t, const TemplateContext *spec, IdmValue *out_value, IdmCore **out_core, IdmError *err) {
    if (spec->mode == TEMPLATE_QUOTE) {
        switch (t->kind) {
            ...
            case IDM_SYN_LIST: {
                ...
                if (idm_syn_is_form_id(t, IDM_FORM_EXPR)) return template_quote_datum_list(ctx, t->as.seq.items, 1, t->as.seq.count, spec, out_value, err);
                if (idm_syn_is_form_id(t, IDM_FORM_GROUP) || idm_syn_is_form_id(t, IDM_FORM_LAYOUT_GROUP)) {
                    if (t->as.seq.count != 2) return idm_error_set(err, t->span, "%-group expects one child");
                    const IdmSyntax *child = t->as.seq.items[1];
                    if (idm_syn_is_form_id(child, IDM_FORM_EXPR)) return template_quote_datum_list(ctx, child->as.seq.items, 1, child->as.seq.count, spec, out_value, err);
                    return template_quote_datum_list(ctx, t->as.seq.items, 1, t->as.seq.count, spec, out_value, err);
                }
                const char *sym = quote_symbol_for_head(t->as.seq.items[0]->form_id);
                if (sym) {
    ...
    if (spec->mode == TEMPLATE_QUASIQUOTE) {
        if (idm_syn_is_form_id(t, IDM_FORM_UNQUOTE)) {
        ...
        if (idm_syn_is_form_id(t, IDM_FORM_UNQUOTE_SPLICING)) {
        ...
        switch (t->kind) {
            ...
            case IDM_SYN_LIST: {
                ...
                if (idm_syn_is_form_id(t, IDM_FORM_EXPR)) {
                ...
                if (idm_syn_is_form_id(t, IDM_FORM_GROUP) || idm_syn_is_form_id(t, IDM_FORM_LAYOUT_GROUP)) {
                    ...
                    const IdmSyntax *child = t->as.seq.items[1];
                    if (idm_syn_is_form_id(child, IDM_FORM_EXPR)) {
    ...
    if (idm_syn_is_form_id(t, IDM_FORM_UNSYNTAX)) {
        if (t->as.seq.count != 2) { expand_error(err, t->span, "unsyntax expects one expression"); return false; }
        *out_core = expand_syntax(ctx, t->as.seq.items[1], err);
        return *out_core != NULL;
    }
    if (idm_syn_is_form_id(t, IDM_FORM_UNSYNTAX_SPLICING)) {
        expand_error(err, t->span, "unsyntax-splicing is only valid inside sequence templates");
        return false;
    }
    if ((idm_syn_is_form_id(t, IDM_FORM_GROUP) || idm_syn_is_form_id(t, IDM_FORM_LAYOUT_GROUP)) && t->as.seq.count == 2 && idm_syn_is_form_id(t->as.seq.items[1], IDM_FORM_EXPR) && t->as.seq.items[1]->as.seq.count == 2 && idm_syn_is_form_id(t->as.seq.items[1]->as.seq.items[1], IDM_FORM_BODY)) {
        *out_core = template_walk_core(ctx, t->as.seq.items[1]->as.seq.items[1], spec, err);
        return *out_core != NULL;
    }
    if (idm_syn_is_form_id(t, IDM_FORM_EXPR) || idm_syn_is_form_id(t, IDM_FORM_BODY)) {
        ...
        IdmSyntaxBuildKind kind = idm_syn_is_form_id(t, IDM_FORM_EXPR) ? IDM_SYNTAX_BUILD_EXPR : IDM_SYNTAX_BUILD_BODY;
...
IdmCore *expand_template_quasisyntax(ExpandContext *ctx, const IdmSyntax *template, const IdmSyntax *template_ctx, IdmError *err) {
...
IdmCore *expand_template_quasiquote(ExpandContext *ctx, const IdmSyntax *t, IdmError *err) {
    return quasiquote_template(ctx, t, err);
}
