...
static bool install_relocated_operator(ExpandContext *ctx, const IdmOperatorDef *op, IdmScopeId min_id, int64_t delta, IdmError *err);
...
IdmCore *expand_use(ExpandContext *ctx, const char *path, const char *qualifier, IdmSyntax *const *items, size_t cont_index, size_t cont_count, IdmSpan span, IdmError *err) {
    ...
    if (import_count != 0) {
        ...
        if (!export_src || !export_dst) { free(export_src); free(export_dst); return (IdmCore *)(uintptr_t)idm_error_oom(err, span); }
    }
    bool ok = true;
    ...
    for (size_t i = 0; i < art->export_count && ok; i++) {
        ...
        ok = idm_binding_table_add(&ctx->bindings, bind_name, 0, IDM_BIND_SPACE_DEFAULT, IDM_BIND_GLOBAL, &ctx->empty_scopes, consumer_slot, IDM_FRAME_GLOBAL, NULL);
        free(qualified);
    }
    for (size_t i = 0; i < art->global_count && ok; i++) {
        ...
        ok = idm_binding_table_add(&ctx->bindings, art->globals[i].name, 0, IDM_BIND_SPACE_DEFAULT, IDM_BIND_GLOBAL, &gscopes, consumer_slot, IDM_FRAME_GLOBAL, NULL);
    ...
    IdmCore *cont = expand_body_items(ctx, items, cont_index, cont_count, err);
    if (!cont) { free(export_src); free(export_dst); return NULL; }

    uint32_t fn_off = 0;
    IdmBytecodeModule *module = relocated_module_copy(ctx, art->module, min_id, delta, &fn_off, err);
    if (!module) {
        free(export_src);
        free(export_dst);
        idm_core_free(cont);
...
bool ctx_activate_kernel(ExpandContext *ctx, IdmError *err) {
    ...
    if (import_count != 0) {
        ...
        for (size_t i = 0; i < k->export_count; i++) {
            ...
            n++;
            if (!idm_binding_table_add(&ctx->bindings, k->exports[i].name, 0, IDM_BIND_SPACE_DEFAULT, IDM_BIND_GLOBAL, &ctx->empty_scopes, consumer_slot, IDM_FRAME_GLOBAL, NULL)) return idm_error_oom(err, idm_span_unknown(NULL));
        }
        for (size_t i = 0; i < k->global_count; i++) {
            ...
            ctx->kernel_import_dst[n] = consumer_slot;
            n++;
            if (!idm_binding_table_add(&ctx->bindings, k->globals[i].name, 0, IDM_BIND_SPACE_DEFAULT, IDM_BIND_GLOBAL, &k->globals[i].scopes, consumer_slot, IDM_FRAME_GLOBAL, NULL)) return idm_error_oom(err, idm_span_unknown(NULL));
        }
...
bool activate_artifact(ExpandContext *ctx, const char *protocol_name, const IdmArtifact *art, IdmScopeId base, IdmSpan span, IdmError *err) {
    (void)span;
    IdmScopeId min_id = art->scope_base;
    int64_t delta = (int64_t)base - (int64_t)art->scope_base;
...
static bool compile_package_artifact(IdmRuntime *rt, IdmScopeStore *store, const IdmSyntax *pkg, const char *protocol_name_hint, bool kernel_mode, IdmArtifact *out, IdmError *err) {
    ...
    IdmCore *body = ok ? expand_body_items(&ctx, scoped_pkg->as.seq.items, 1, scoped_pkg->as.seq.count, err) : NULL;
    ...
    if (copy_ok && method_count != 0) {
        ...
        if (!methods) copy_ok = false;
            ...
            for (size_t i = 0; i < method_count; i++) {
                methods[i].name = idm_strdup(ctx.decl_methods[i].name);
                ...
                if (!methods[i].name || !idm_scope_set_copy(&methods[i].scopes, &ctx.decl_methods[i].scopes)) {
