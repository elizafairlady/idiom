...
const IdmOpcodeInfo idm_opcode_infos[IDM_OP_COUNT] = {
#define IDM_OPCODE_INFO(name, fixed, count_index, repeat, roles, repeat_roles, reg_roles, reg_ranges, flow_kind, branch, tail, selector_site) \
    [IDM_OP_##name] = {#name, fixed, count_index, repeat, roles, repeat_roles, reg_roles, reg_ranges, flow_kind, branch, tail, selector_site},
...
static void code_sites_clear(IdmBytecodeModule *module) {
    for (size_t i = 0; i < module->code_site_count; i++) {
        idm_pattern_selector_free(module->code_sites[i].selector);
        free(module->code_sites[i].record.contracts);
    }
    free(module->code_sites);
    module->code_sites = NULL;
    module->code_site_count = 0;
...
static void selectors_invalidate(IdmBytecodeModule *module) {
    if (module->selectors_prepared || module->code_sites || module->code_site_count != 0) {
...
bool idm_bc_set_function_doc(IdmBytecodeModule *module, uint32_t function_index, const char *doc) {
    if (!module || function_index >= module->function_count) return false;
    if (!doc || !doc[0]) return true;
    char *copy = idm_strdup(doc);
    if (!copy) return false;
    free(module->functions[function_index].doc);
    module->functions[function_index].doc = copy;
    return true;
}
...
void idm_bc_init(IdmBytecodeModule *module) {
    ...
    module->code_sites = NULL;
    module->code_site_count = 0;
    module->env_closures = NULL;
    module->env_closure_count = 0;
    module->env_closure_cap = 0;
    module->selectors_prepared = false;
    module->literals_interned = false;
    module->verified = false;
    ...
    switch (pat->kind) {
        ...
        case IDM_PAT_BITS:
            for (size_t i = 0; i < pat->as.bits.count; i++)
                if (!intern_pattern_literals(rt, pat->as.bits.segs[i].sub, err)) return false;
            return true;
        default:
            return true;
    }
...
bool idm_bc_add_function(IdmBytecodeModule *module, const char *name, uint32_t arity, uint32_t local_count, size_t entry, uint32_t *out_index) {
    ...
    memset(&module->functions[index], 0, sizeof(module->functions[index]));
...
static bool selector_site_add_take(IdmBytecodeModule *module, size_t offset, IdmPatternSelector *selector, IdmError *err) {
    ...
    if (offset >= module->code_site_count) {
        idm_pattern_selector_free(selector);
        return idm_error_set(err, idm_span_unknown(NULL), "selector site offset is out of bounds");
    }
    if (module->code_sites[offset].selector) {
        idm_pattern_selector_free(selector);
        return idm_error_set(err, idm_span_unknown(NULL), "duplicate selector site at offset %zu", offset);
    }
    module->code_sites[offset].selector = selector;
    return true;
}
...
static bool selector_site_for_instr(IdmRuntime *rt, IdmBytecodeModule *module, const IdmBcInstr *instr, IdmError *err) {
    if (!instr->info->selector_site) return true;
    ...
    if (fixed_count == 0 && repeat_functions == 0) return idm_error_set(err, idm_span_unknown(NULL), "%s has selector-site metadata but no function operands", instr->info->name);
...
IdmPatternSelector *idm_bc_selector_at(const IdmBytecodeModule *module, size_t offset) {
    if (!module || offset >= module->code_site_count) return NULL;
    return module->code_sites[offset].selector;
}

IdmPatternSelector *idm_bc_function_selector_at(const IdmBytecodeModule *module, uint32_t fn) {
    if (!module || fn >= module->function_count) return NULL;
    return module->functions[fn].selector;
}

static bool bc_const_symbol(IdmRuntime *rt, const IdmBytecodeModule *module, uint32_t index, const char *what, IdmSymbol **out, IdmError *err) {
    if (index >= module->const_count) return idm_error_set(err, idm_span_unknown(NULL), "%s constant %u out of bounds", what, index);
    IdmValue value = module->constants[index];
    if (idm_value_tag(value) == IDM_VAL_ATOM || idm_value_tag(value) == IDM_VAL_WORD) {
        *out = idm_value_symbol(value);
        return true;
    }
    if (idm_value_tag(value) == IDM_VAL_STRING) {
        *out = idm_intern(&rt->intern, IDM_SYMBOL_ATOM, idm_string_bytes(value));
        return *out ? true : idm_error_oom(err, idm_span_unknown(NULL));
    }
    return idm_error_set(err, idm_span_unknown(NULL), "%s constant must be a name", what);
}

static bool bc_type_term(const IdmBytecodeModule *module, uint32_t index, const char *what, const IdmTypeTerm **out, IdmError *err) {
    if (index == UINT32_MAX) {
        *out = NULL;
        return true;
    }
    if (index >= module->type_count) return idm_error_set(err, idm_span_unknown(NULL), "%s type %u out of bounds", what, index);
    *out = &module->types[index];
    return true;
}
...
static bool record_site_for_instr(IdmRuntime *rt, IdmBytecodeModule *module, const IdmBcInstr *instr, IdmError *err) {
    ...
    if (field_count != 0 && !contracts) {
        free(fields);
        return idm_error_oom(err, idm_span_unknown(NULL));
    }
    bool ok = true;
    for (uint32_t i = 0; i < field_count && ok; i++) {
        uint32_t field_const = instr->repeat_operands[(size_t)i * 2u];
        uint32_t contract_index = instr->repeat_operands[(size_t)i * 2u + 1u];
        ok = bc_const_symbol(rt, module, field_const, "MAKE_RECORD field", &fields[i], err) &&
             bc_type_term(module, contract_index, "MAKE_RECORD field contract", &contracts[i], err);
    }
    IdmRecordShape *shape = ok ? idm_record_shape_intern_symbols(rt, type, fields, field_count, err) : NULL;
    if (ok && shape) ok = idm_record_shape_fill_contracts(shape, contracts, err);
    if (ok && shape) {
        IdmBcRecordSite *site = &module->code_sites[instr->offset].record;
        site->shape = shape;
        site->contracts = contracts;
        site->field_count = field_count;
        contracts = NULL;
    }
    free(contracts);
    free(fields);
    return ok && !(err && err->present);
}

static bool prepare_function_selectors(IdmRuntime *rt, IdmBytecodeModule *module, IdmError *err) {
    for (uint32_t i = 0; i < module->function_count; i++) {
        uint32_t entry = i;
        if (!idm_bc_build_selector_for_entries(rt, module, &entry, 1u, &module->functions[i].selector, err)) return false;
    }
    return true;
}

static bool prepare_instruction_selectors(IdmRuntime *rt, IdmBytecodeModule *module, IdmError *err) {
    size_t ip = 0;
    while (ip < module->code_count) {
        IdmBcInstr instr;
        if (!decode_instr(module, ip, &instr, err)) return false;
        if (!selector_site_for_instr(rt, module, &instr, err)) return false;
        if (!record_site_for_instr(rt, module, &instr, err)) return false;
        ip += instr.width;
    }
    return true;
}
...
bool idm_bc_prepare_selectors(IdmRuntime *rt, IdmBytecodeModule *module, IdmError *err) {
    ...
    selectors_clear(module);
    module->code_sites = module->code_count == 0 ? NULL : calloc(module->code_count, sizeof(*module->code_sites));
    if (module->code_count != 0 && !module->code_sites) {
        idm_profile_scope_end(&prof);
        return idm_error_oom(err, idm_span_unknown(NULL));
    }
    module->code_site_count = module->code_count;
    if (!prepare_function_selectors(rt, module, err) || !prepare_instruction_selectors(rt, module, err)) {
        selectors_clear(module);
        idm_profile_scope_end(&prof);
        return false;
    }
...
static bool verify_reference_role(const IdmBytecodeModule *module, const IdmBcInstr *instr, char role, uint32_t word, size_t *targets, size_t *target_count, IdmError *err) {
    switch (role) {
        ...
        case 'j':
            ...
            if (word <= instr->offset) return idm_error_set(err, idm_span_unknown(NULL), "%s target %u is not forward", instr->info->name, word);
...
static bool verify_match_bind_shape(const IdmBytecodeModule *module, const IdmBcInstr *instr, IdmError *err) {
    if (instr->op != IDM_OP_MATCH_BIND) return true;
    uint32_t fn_index = instr->operands[0];
    uint32_t count = instr->operands[3];
    if (fn_index >= module->function_count) return true;
    const IdmBcFunction *fn = &module->functions[fn_index];
    if (fn->pattern_local_count != count) {
        return idm_error_set(err, idm_span_unknown(NULL), "MATCH_BIND binding count %u does not match function pattern locals %u", count, fn->pattern_local_count);
    }
    for (uint32_t i = 0; i < fn->pattern_local_count; i++) {
        if (fn->pattern_locals[i].slot != i) {
            return idm_error_set(err, idm_span_unknown(NULL), "MATCH_BIND pattern local slot %u does not match binding index %u", fn->pattern_locals[i].slot, i);
        }
    }
    return true;
}
...
static bool verify_scan(const IdmBytecodeModule *module, unsigned char *is_start, size_t *targets, size_t *target_count, IdmError *err) {
    ...
    while (ip < module->code_count) {
        ...
        if (!verify_match_bind_shape(module, &instr, err)) return false;
        ip += instr.width;
    }
    return true;
}
...
static bool serialize_value(IdmBuffer *out, IdmValue v, unsigned depth, IdmError *err) {
    ...
    switch (idm_value_tag(v)) {
        ...
        case IDM_VAL_BITSTRING: {
            uint64_t len = idm_bits_len(v);
            size_t nbytes = (size_t)((len + 7u) / 8u);
            unsigned char *bytes = malloc(nbytes ? nbytes : 1u);
            if (!bytes) return idm_error_oom(err, idm_span_unknown(NULL));
            idm_bits_read(v, 0u, len, bytes);
            bool ok = idm_buf_put_u8(out, 17u) &&
                      idm_buf_put_u64(out, len) &&
                      idm_buf_put_str(out, (const char *)bytes, nbytes);
            free(bytes);
...
static bool serialize_pattern(IdmBuffer *out, const IdmPattern *pat, unsigned depth, IdmError *err) {
    ...
    switch (pat->kind) {
        ...
        case IDM_PAT_BITS: {
            if (!idm_buf_put_u32(out, (uint32_t)pat->as.bits.count)) return idm_error_oom(err, idm_span_unknown(NULL));
            for (size_t i = 0; i < pat->as.bits.count; i++) {
                const IdmBitSeg *seg = &pat->as.bits.segs[i];
                uint8_t flags = (uint8_t)((seg->little ? 1u : 0u) | (seg->is_signed ? 2u : 0u) | (seg->size_is_slot ? 4u : 0u));
                if (!idm_buf_put_u8(out, (uint8_t)seg->kind) ||
                    !idm_buf_put_u8(out, flags) ||
                    !idm_buf_put_u32(out, seg->size_slot) ||
                    !idm_buf_put_opt_str(out, seg->size_name) ||
                    !idm_buf_put_u64(out, seg->size) ||
                    !idm_buf_put_u8(out, seg->sub ? 1u : 0u)) return idm_error_oom(err, idm_span_unknown(NULL));
                if (seg->sub && !serialize_pattern(out, seg->sub, depth + 1u, err)) return false;
            }
            return true;
        }
        default:
...
bool idm_ic_serialize(const IdmBytecodeModule *module, IdmBuffer *out, IdmError *err) {
    ...
    for (size_t i = 0; i < module->function_count; i++) {
        ...
        if (!idm_buf_put_opt_str(out, f->doc)) return idm_error_oom(err, idm_span_unknown(NULL));
...
static bool deserialize_value(IdmRuntime *rt, IdmByteReader *r, IdmValue *out, unsigned depth, IdmError *err) {
    ...
    switch (tag) {
        ...
        case 17u: {
            ...
            if (!r->ok) return idm_error_set(err, idm_span_unknown(NULL), "truncated bitstring");
            size_t nbytes = 0;
            char *bytes = idm_rd_string(r, &nbytes);
            if (!bytes) return idm_error_set(err, idm_span_unknown(NULL), "truncated bitstring");
            if (len > UINT64_MAX - 7u || (uint64_t)nbytes != (len + 7u) / 8u) {
                free(bytes);
                return idm_error_set(err, idm_span_unknown(NULL), "invalid bitstring length");
            }
            *out = idm_bits_from_bytes(rt, (const unsigned char *)bytes, len, err);
            free(bytes);
            return !(err && err->present);
        }
        default:
...
static IdmPattern *deserialize_pattern(IdmRuntime *rt, IdmByteReader *r, unsigned depth, IdmError *err) {
    ...
    switch (kind) {
        ...
        case IDM_PAT_BITS: {
            uint32_t count = idm_rd_u32(r);
            if (!r->ok) { idm_error_set(err, span, "truncated .ic bitstring pattern"); return NULL; }
            IdmBitSeg *segs = count == 0 ? NULL : calloc(count, sizeof(*segs));
            if (count != 0 && !segs) { idm_error_oom(err, span); return NULL; }
            for (uint32_t i = 0; i < count; i++) {
                uint8_t seg_kind = idm_rd_u8(r);
                uint8_t flags = idm_rd_u8(r);
                segs[i].kind = (IdmBitSegKind)seg_kind;
                segs[i].little = (flags & 1u) != 0;
                segs[i].is_signed = (flags & 2u) != 0;
                segs[i].size_is_slot = (flags & 4u) != 0;
                segs[i].size_slot = idm_rd_u32(r);
                if (!idm_rd_opt_str(r, &segs[i].size_name, err)) {
                    for (uint32_t j = 0; j < i; j++) { idm_pat_free(segs[j].sub); free(segs[j].size_name); }
                    free(segs);
                    return NULL;
                }
                segs[i].size = idm_rd_u64(r);
                uint8_t has_sub = idm_rd_u8(r);
                if (!r->ok || seg_kind > IDM_BITSEG_REST) {
                    for (uint32_t j = 0; j <= i; j++) { idm_pat_free(segs[j].sub); free(segs[j].size_name); }
                    free(segs);
                    idm_error_set(err, span, "truncated .ic bitstring pattern");
                    return NULL;
                }
                if (has_sub) {
                    segs[i].sub = deserialize_pattern(rt, r, depth + 1u, err);
                    if (!segs[i].sub) {
                        for (uint32_t j = 0; j <= i; j++) { idm_pat_free(segs[j].sub); free(segs[j].size_name); }
                        free(segs);
                        return NULL;
            ...
            IdmPattern *p = idm_pat_bits_take(segs, count, span);
            if (!p) {
                for (uint32_t j = 0; j < count; j++) { idm_pat_free(segs[j].sub); free(segs[j].size_name); }
                free(segs);
                idm_error_oom(err, span);
            }
            return p;
...
bool idm_ic_deserialize(IdmRuntime *rt, const unsigned char *data, size_t len, IdmBytecodeModule *module, IdmError *err) {
    ...
    for (uint32_t i = 0; i < function_count && r.ok; i++) {
        ...
        char *fdoc = NULL;
        if (!idm_rd_opt_str(&r, &fdoc, err)) { free(name); free(deferred_guards); idm_bc_destroy(module); return false; }
        ...
        if (!r.ok || !added) { free(name); free(fdoc); free(deferred_guards); idm_bc_destroy(module); return idm_error_set(err, idm_span_unknown(NULL), "corrupt .ic function"); }
        module->functions[idx].call_arity = call_arity;
        module->functions[idx].register_count = register_count;
        module->functions[idx].doc = fdoc;
        free(name);
...
bool idm_bc_link(IdmBytecodeModule *dst, const IdmBytecodeModule *src, uint32_t *out_const_offset, uint32_t *out_fn_offset, uint32_t *out_code_offset, IdmError *err) {
    ...
    for (size_t i = 0; i < src->function_count; i++) {
        ...
        if (sf->doc && !idm_bc_set_function_doc(dst, nf, sf->doc)) return idm_error_oom(err, idm_span_unknown(NULL));
...
typedef struct { uint32_t entry; uint32_t index; } ShakeEnt;

static int shake_ent_cmp(const void *a, const void *b) {
    uint32_t x = ((const ShakeEnt *)a)->entry, y = ((const ShakeEnt *)b)->entry;
    return x < y ? -1 : (x > y ? 1 : 0);
}

static void shake_free_function(IdmBcFunction *f) {
    free(f->name);
    free(f->doc);
    for (uint32_t p = 0; p < f->pattern_count; p++) idm_pat_free(f->param_patterns[p]);
    free(f->param_patterns);
    for (uint32_t p = 0; p < f->pattern_local_count; p++) free(f->pattern_locals[p].name);
    free(f->pattern_locals);
}

bool idm_bc_tree_shake(IdmBytecodeModule *module, uint32_t *main_fn, IdmError *err) {
    IdmProfileScope prof;
    idm_profile_scope_begin(&prof, "bytecode.tree_shake");
    size_t fc = module->function_count;
    if (fc == 0 || !main_fn || *main_fn >= fc) { idm_profile_scope_end(&prof); return true; }
    selectors_clear(module);

    ShakeEnt *ents = malloc(fc * sizeof(*ents));
    size_t *fn_end = malloc(fc * sizeof(*fn_end));
    bool *live = calloc(fc, sizeof(*live));
    uint32_t *stack = malloc(fc * sizeof(*stack));
    uint32_t *old_to_new = malloc(fc * sizeof(*old_to_new));
    size_t *new_entry = malloc(fc * sizeof(*new_entry));
    size_t *off_map = malloc((module->code_count == 0 ? 1u : module->code_count) * sizeof(*off_map));
    if (!ents || !fn_end || !live || !stack || !old_to_new || !new_entry || !off_map) {
        free(ents); free(fn_end); free(live); free(stack); free(old_to_new); free(new_entry); free(off_map);
        idm_profile_scope_end(&prof);
        return idm_error_oom(err, idm_span_unknown(NULL));
    }
    bool ok = true;
    ...
    for (size_t i = 0; i < fc; i++) {
        if (!module->functions[i].primitive_backed) { ents[ne].entry = (uint32_t)module->functions[i].entry; ents[ne].index = (uint32_t)i; ne++; }
    }
    qsort(ents, ne, sizeof(*ents), shake_ent_cmp);
    for (size_t k = 0; k < ne && ok; k++) {
        uint32_t f = ents[k].index;
        size_t end = (k + 1 < ne) ? (size_t)ents[k + 1].entry : module->code_count;
        if ((size_t)module->functions[f].entry > end) ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: non-contiguous function code");
        fn_end[f] = end;
    }

    size_t sp = 0;
    if (ok) { live[*main_fn] = true; stack[sp++] = *main_fn; }
    while (ok && sp > 0) {
        uint32_t f = stack[--sp];
        IdmBcFunction *fn = &module->functions[f];
        if (fn->has_guard) {
            uint32_t g = fn->guard_function;
            if (g >= fc) { ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: guard out of bounds"); break; }
            if (!live[g]) { live[g] = true; stack[sp++] = g; }
        }
        if (fn->primitive_backed) continue;
        size_t ip = fn->entry, end = fn_end[f];
        while (ok && ip < end) {
            const IdmOpcodeInfo *info = idm_opcode_info((IdmOpcode)module->code[ip]);
            if (!info) { ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: invalid opcode"); break; }
            uint32_t repeat_count = 0;
            for (uint8_t i = 0; i < info->fixed_operands; i++) {
                uint32_t w = module->code[ip + 1u + i];
                if (info->count_operand == i) repeat_count = w;
                if (opcode_role_at(info->operand_roles, i) == 'f') {
                    if (w >= fc) { ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: function ref out of bounds"); break; }
                    if (!live[w]) { live[w] = true; stack[sp++] = w; }
                }
            }
            size_t width = 1u + info->fixed_operands;
            if (ok && info->count_operand != IDM_OPCODE_NO_COUNT) {
                size_t base = ip + 1u + info->fixed_operands;
                for (uint32_t item = 0; ok && item < repeat_count; item++) {
                    for (uint8_t part = 0; part < info->repeat_width; part++) {
                        if (opcode_role_at(info->repeat_roles, part) != 'f') continue;
                        uint32_t w = module->code[base + (size_t)item * info->repeat_width + part];
                        if (w >= fc) { ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: function ref out of bounds"); break; }
                        if (!live[w]) { live[w] = true; stack[sp++] = w; }
                    }
                }
                width += (size_t)repeat_count * info->repeat_width;
            }
            ip += width;
        }
    }
    ...
    size_t live_count = 0;
    for (size_t i = 0; ok && i < fc; i++) if (live[i]) live_count++;
    if (!ok || live_count == fc) {
        free(ents); free(fn_end); free(live); free(stack); free(old_to_new); free(new_entry); free(off_map);
        idm_profile_scope_end(&prof);
        return ok;
    ...
    uint32_t new_fc = 0;
    for (size_t i = 0; i < fc; i++) old_to_new[i] = live[i] ? new_fc++ : UINT32_MAX;
    for (size_t i = 0; i < module->code_count; i++) off_map[i] = SIZE_MAX;
    size_t new_code_count = 0;
    for (size_t k = 0; k < ne; k++) { uint32_t f = ents[k].index; if (live[f]) new_code_count += fn_end[f] - (size_t)module->functions[f].entry; }
    uint32_t *new_code = new_code_count == 0 ? NULL : malloc(new_code_count * sizeof(*new_code));
    IdmBcFunction *new_fns = malloc((size_t)new_fc * sizeof(*new_fns));
    if ((new_code_count != 0 && !new_code) || !new_fns) {
        free(new_code); free(new_fns);
        free(ents); free(fn_end); free(live); free(stack); free(old_to_new); free(new_entry); free(off_map);
        idm_profile_scope_end(&prof);
        return idm_error_oom(err, idm_span_unknown(NULL));
    }
    ...
    for (size_t k = 0; k < ne; k++) {
        uint32_t f = ents[k].index;
        if (!live[f]) continue;
        size_t oe = module->functions[f].entry, e = fn_end[f];
        new_entry[f] = nc;
        for (size_t ip = oe; ip < e; ip++) { off_map[ip] = nc; new_code[nc++] = module->code[ip]; }
    }

    size_t ip = 0;
    while (ok && ip < new_code_count) {
        const IdmOpcodeInfo *info = idm_opcode_info((IdmOpcode)new_code[ip]);
        if (!info) { ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: invalid opcode"); break; }
        uint32_t repeat_count = 0;
        for (uint8_t i = 0; ok && i < info->fixed_operands; i++) {
            size_t idx = ip + 1u + i;
            uint32_t w = new_code[idx];
            if (info->count_operand == i) repeat_count = w;
            char role = opcode_role_at(info->operand_roles, i);
            if (role == 'f') new_code[idx] = old_to_new[w];
            else if (role == 'j') { if (w >= module->code_count || off_map[w] == SIZE_MAX) ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: dangling jump"); else new_code[idx] = (uint32_t)off_map[w]; }
        }
        size_t width = 1u + info->fixed_operands;
        if (ok && info->count_operand != IDM_OPCODE_NO_COUNT) {
            size_t base = ip + 1u + info->fixed_operands;
            for (uint32_t item = 0; ok && item < repeat_count; item++) {
                for (uint8_t part = 0; ok && part < info->repeat_width; part++) {
                    size_t idx = base + (size_t)item * info->repeat_width + part;
                    char role = opcode_role_at(info->repeat_roles, part);
                    if (role == 'f') new_code[idx] = old_to_new[new_code[idx]];
                    else if (role == 'j') { uint32_t w = new_code[idx]; if (w >= module->code_count || off_map[w] == SIZE_MAX) ok = idm_error_set(err, idm_span_unknown(NULL), "tree-shake: dangling jump"); else new_code[idx] = (uint32_t)off_map[w]; }
                }
            }
            width += (size_t)repeat_count * info->repeat_width;
        }
    ...
    for (size_t i = 0; i < fc; i++) {
        if (live[i]) {
            uint32_t ni = old_to_new[i];
            new_fns[ni] = module->functions[i];
            new_fns[ni].entry = module->functions[i].primitive_backed ? 0u : new_entry[i];
            if (new_fns[ni].has_guard) new_fns[ni].guard_function = old_to_new[module->functions[i].guard_function];
            new_fns[ni].selector = NULL;
        } else {
            shake_free_function(&module->functions[i]);
        }
    }
    free(module->functions);
    module->functions = new_fns;
    module->function_count = new_fc;
    module->function_cap = new_fc;
    size_t old_code_count = module->code_count;
    free(module->code);
    module->code = new_code;
    module->code_count = new_code_count;
    module->code_cap = new_code_count;

    size_t sw = 0;
    for (size_t i = 0; i < module->span_count; i++) {
        uint32_t o = module->spans[i].offset;
        size_t no = o < old_code_count ? off_map[o] : SIZE_MAX;
        if (no == SIZE_MAX) continue;
        module->spans[sw] = module->spans[i];
        module->spans[sw].offset = (uint32_t)no;
        sw++;
    }
    module->span_count = sw;

    size_t nw = 0;
    for (size_t i = 0; i < module->name_note_count; i++) {
        uint32_t o = module->name_notes[i].offset;
        size_t no = o < old_code_count ? off_map[o] : SIZE_MAX;
        if (no == SIZE_MAX) { free(module->name_notes[i].name); continue; }
        module->name_notes[nw] = module->name_notes[i];
        module->name_notes[nw].offset = (uint32_t)no;
        nw++;
    }
    module->name_note_count = nw;

    *main_fn = old_to_new[*main_fn];

    free(ents); free(fn_end); free(live); free(stack); free(old_to_new); free(new_entry); free(off_map);
    module->verified = false;
    idm_profile_count("bytecode.tree_shake.functions_kept", (uint64_t)new_fc);
    idm_profile_scope_end(&prof);
    return true;
}
