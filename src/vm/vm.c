...
typedef enum {
    REDUCTION_CONTINUE,
    REDUCTION_YIELD_BEFORE,
    REDUCTION_YIELD_AFTER
} ReductionCharge;
...
static bool closure_info_selector_ready(const IdmClosureInfo *info, IdmError *err) {
    if (!info || !idm_bc_is_finalized(info->module)) return idm_error_set(err, idm_span_unknown(NULL), "closure module is not finalized");
    if (info->selector_generation != info->module->selector_generation) return idm_error_set(err, idm_span_unknown(NULL), "closure selector is stale");
    return true;
}

static bool closure_info_accepts_arity(const IdmClosureInfo *info, uint32_t argc, uint32_t *out_min, uint32_t *out_max) {
    if (!info || !idm_bc_is_finalized(info->module) || info->entry_count == 0) return false;
    uint32_t min = UINT32_MAX;
    uint32_t max = 0;
    bool accepted = false;
    for (size_t i = 0; i < info->entry_count; i++) {
        uint32_t index = info->entry_count == 1u ? info->function_index : info->entries[i];
        if (index >= info->module->function_count) return false;
        const IdmArity *arity = &info->module->functions[index].call_arity;
        if (arity->kind == IDM_ARITY_UNKNOWN) continue;
        if (arity->min < min) min = arity->min;
        if (arity->max > max) max = arity->max;
        if (idm_arity_accepts(arity, argc)) accepted = true;
    }
...
static bool closure_selector_ready(Vm *vm, IdmValue closure, IdmError *err) {
    (void)vm;
    IdmClosureInfo info;
    if (!idm_closure_info(closure, &info)) return idm_error_set(err, idm_span_unknown(NULL), "expected closure");
    return closure_info_selector_ready(&info, err);
}
...
static bool vm_spend_reduction(Vm *vm, IdmExecStatus *status, ReductionCharge *out, IdmError *err) {
    *out = REDUCTION_CONTINUE;
    if (!vm->budget || *vm->budget < 0) return true;
    if (!status) return idm_error_set(err, idm_span_unknown(NULL), "reduction budget requires a schedulable VM step");
    if (*vm->budget == 0) {
        if (*status == IDM_EXEC_DONE) *status = IDM_EXEC_YIELD;
        *out = REDUCTION_YIELD_BEFORE;
        return true;
    }
    (*vm->budget)--;
    if (*vm->budget == 0) {
        if (*status == IDM_EXEC_DONE) *status = IDM_EXEC_YIELD;
        *out = REDUCTION_YIELD_AFTER;
    }
    return true;
}

static void rewind_current_frame(Vm *vm, size_t ip) {
    if (ip == SIZE_MAX) return;
...
static bool op_port_read_block(Vm *vm, IdmPrimitive primitive, const IdmValue *args, uint32_t argc, IdmError *err) {
    ...
    if (strcmp(stream, "stderr") == 0 && idm_sched_port_stderr_merged(vm->sched, idm_value_id(port))) {
        idm_error_set(err, idm_span_unknown(NULL), "port-read :stderr on a pty port: stderr is merged into the pty; read :stdout");
        return idm_error_reason(vm->rt, err, "type-error", 2, idm_atom(vm->rt, "port-read"), streamv);
    }
    int64_t max = 0;
...
static bool blocking_dispatch(Vm *vm, IdmExecStatus *status, uint32_t dst, IdmPrimitive primitive, BlockingOp op, IdmExecStatus block_status, const IdmValue *args, uint32_t argc, bool meter, size_t rewind_ip, IdmError *err) {
    ...
    if (meter) {
        ReductionCharge charge;
        if (!vm_spend_reduction(vm, status, &charge, err)) return false;
        if (charge == REDUCTION_YIELD_BEFORE) {
            rewind_current_frame(vm, rewind_ip);
            return true;
        }
    }
...
static bool call_value(Vm *vm, uint32_t dst, IdmValue callee, const IdmValue *args, uint32_t argc, bool tail, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err);
...
static bool execute_apply_primitive(Vm *vm, uint32_t dst, const IdmValue *args, uint32_t argc, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    ...
    bool ok = call_value(vm, dst, callee, apply_args, (uint32_t)apply_argc, false, rewind_ip, status, out_reason, err);
    free(apply_args);
    return ok;
}
...
static bool execute_actor_primitive(Vm *vm, IdmPrimitive primitive, const IdmValue *args, uint32_t argc, IdmExecStatus *status, IdmValue *out, IdmValue *out_reason, IdmError *err) {
    ...
    switch (primitive) {
        ...
        case IDM_PRIM_PROCESSES:
            return idm_sched_processes_value(vm->sched, vm->rt, out, err);
        case IDM_PRIM_PROCESS_INFO: {
            if (idm_value_tag(args[0]) != IDM_VAL_PID) {
                if (idm_value_tag(args[0]) == IDM_VAL_PORT) {
                    *out = idm_nil();
                    return true;
                }
                return idm_error_set(err, idm_span_unknown(NULL), "process-info expects a pid or a port");
            }
            return idm_sched_process_info_value(vm->sched, vm->rt, idm_value_id(args[0]), out, err);
        }
        case IDM_PRIM_PORT_INFO: {
            if (idm_value_tag(args[0]) != IDM_VAL_PORT) {
                ...
                return idm_error_set(err, idm_span_unknown(NULL), "port-info expects a pid or a port");
            }
            return idm_sched_port_info_value(vm->sched, vm->rt, idm_value_id(args[0]), out, err);
        }
        case IDM_PRIM_EXIT: {
            if (argc == 2u) {
                IdmValue target = args[0];
...
static bool frame_reg_value(const Frame *frame, uint32_t reg, IdmValue *out, IdmError *err) {
    (void)err;
    IDM_ASSERT_PROVED(reg < frame->reg_count);
    *out = frame->regs[reg];
    return true;
}
...
static bool frame_reg_slice(Frame *frame, uint32_t first, uint32_t count, const IdmValue **out, IdmError *err) {
    ...
    IDM_ASSERT_PROVED(first < frame->reg_count && count <= frame->reg_count - first);
...
static bool execute_primitive_clause(Vm *vm, uint32_t primitive, uint32_t dst, const IdmValue *args, uint32_t argc, bool meter_blocking, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    if (vm->self && vm->sched && (primitive == IDM_PRIM_TTY_READ || primitive == IDM_PRIM_TTY_READ_LINE)) {
        return blocking_dispatch(vm, status, dst, (IdmPrimitive)primitive, op_tty_block, IDM_EXEC_BLOCK_TTY, args, argc, meter_blocking, rewind_ip, err);
    }
    if (vm->self && vm->sched && primitive == IDM_PRIM_PORT_READ) {
        return blocking_dispatch(vm, status, dst, (IdmPrimitive)primitive, op_port_read_block, IDM_EXEC_BLOCK_PORT_READ, args, argc, meter_blocking, rewind_ip, err);
    }
    if (vm->self && vm->sched && primitive == IDM_PRIM_PORT_WRITE) {
        return blocking_dispatch(vm, status, dst, (IdmPrimitive)primitive, op_port_write_block, IDM_EXEC_BLOCK_PORT_WRITE, args, argc, meter_blocking, rewind_ip, err);
    }
    IdmValue out = idm_nil();
    switch ((IdmPrimitive)primitive) {
        case IDM_PRIM_APPLY:
            return execute_apply_primitive(vm, dst, args, argc, rewind_ip, status, out_reason, err);
        case IDM_PRIM_SELF:
        case IDM_PRIM_SPAWN:
        case IDM_PRIM_SPAWN_LINK:
        case IDM_PRIM_SPAWN_MONITOR:
...
static bool vm_run_loop_handlers(Vm *vm, int64_t *budget, IdmExecStatus *status, IdmValue *out_result, IdmValue *out_reason, IdmError *err);
...
static bool enter_clause(Vm *vm, const IdmBytecodeModule *module, uint32_t function_index, IdmValue closure, IdmEnv *env, const IdmValue *captures, uint32_t capture_count, uint32_t dst, const IdmValue *args, uint32_t argc, const IdmPatternBindings *bindings, bool tail, bool meter, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err);
...
static bool vm_setup_closure(Vm *vm, const void *user, IdmError *err) {
    ...
    return call_value(vm, 0, setup->closure, setup->args, setup->argc, false, SIZE_MAX, NULL, NULL, err);
}

static bool vm_bootstrap_run_budgeted(IdmRuntime *rt, const IdmBytecodeModule *module, IdmVmLimits limits, IdmEnv *env, VmBootstrapSetupFn setup_fn, const void *setup, int64_t budget, IdmExecStatus *out_status, const char *outside_message, IdmValue *out, IdmError *err) {
    Vm vm;
    ...
    vm.limits = limits;
    vm.root_env = env;
    vm.strict_guards = budget >= 0;
    if (!setup_fn(&vm, setup, err)) {
        vm_reset(&vm);
        return false;
    }
    return vm_run_call_with_exit_message(&vm, budget, out_status, out, outside_message, err);
}
...
static bool vm_bootstrap_run(IdmRuntime *rt, const IdmBytecodeModule *module, IdmVmLimits limits, IdmEnv *env, VmBootstrapSetupFn setup_fn, const void *setup, const char *outside_message, IdmValue *out, IdmError *err) {
    return vm_bootstrap_run_budgeted(rt, module, limits, env, setup_fn, setup, -1, NULL, outside_message, out, err);
}
...
static bool run_guard_function(Vm *caller, const IdmBytecodeModule *module, IdmValue closure, IdmEnv *env, const IdmValue *captures, uint32_t capture_count, const IdmBcFunction *candidate, const IdmPatternBindings *bindings, ClauseArgs args, bool *out_pass, bool *out_exhausted, IdmError *err) {
    *out_pass = true;
    ...
    guard_vm.strict_guards = caller->strict_guards;
    guard_vm.self = caller->self;
    guard_vm.sched = caller->sched;
    guard_vm.root_env = env ? env : (caller->root_env ? caller->root_env : caller->rt->main_env);
    if (!enter_clause(&guard_vm, module, candidate->guard_function, closure, env, captures, capture_count, 0u, args.values, args.count, bindings, false, false, SIZE_MAX, NULL, NULL, err)) { vm_reset(&guard_vm); return false; }
    ...
    if (status != IDM_EXEC_DONE) {
        if (caller->strict_guards) {
            *out_pass = false;
            return idm_error_set(err, idm_span_unknown(NULL), "guard blocked outside the scheduler");
        }
        *out_pass = false;
        return true;
    }
...
static bool select_ready_closure_clause(Vm *vm, IdmValue callee, const IdmClosureInfo *info, ClauseArgs args, ClauseSelection *selection, IdmError *err) {
    clause_selection_init(selection);
    ...
    return run_selector(vm, info->selector, &guard_ctx, selection, err);
}

static bool select_closure_clause(Vm *vm, IdmValue callee, const IdmClosureInfo *info, ClauseArgs args, ClauseSelection *selection, IdmError *err) {
    if (!closure_info_selector_ready(info, err)) return false;
    return select_ready_closure_clause(vm, callee, info, args, selection, err);
}
...
static bool enter_clause(Vm *vm, const IdmBytecodeModule *module, uint32_t function_index, IdmValue closure, IdmEnv *env, const IdmValue *captures, uint32_t capture_count, uint32_t dst, const IdmValue *args, uint32_t argc, const IdmPatternBindings *bindings, bool tail, bool meter, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    ...
    if (fn->primitive_backed) {
        ...
        return execute_primitive_clause(vm, fn->primitive, dst, args, argc, false, SIZE_MAX, status, out_reason, err);
    }
...
static bool execute_match_op(Vm *vm, Frame *frame, const IdmBytecodeModule *module, size_t instr_ip, const uint32_t *operands, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    ...
    IDM_ASSERT_PROVED(entry_count > 0);
    ...
    bool entered = enter_clause(vm, module, selected.function_index, idm_nil(), frame->env, captures, capture_count, dst, args, argc, selected.has_bindings ? &selected.bindings : NULL, tail, true, instr_ip, status, out_reason, err);
    clause_selection_destroy(&selected);
    return entered;
}

static bool execute_call_op(Vm *vm, Frame *frame, size_t instr_ip, const uint32_t *operands, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    uint32_t dst = operands[0];
    ...
    const IdmValue *args = NULL;
    if (!frame_reg_slice(frame, first_arg, argc, &args, err)) return false;
    return call_value(vm, dst, callee, args, argc, tail, instr_ip, status, out_reason, err);
}

static bool call_closure_clause(Vm *vm, uint32_t dst, IdmValue callee, const IdmClosureInfo *info, const IdmValue *arg_values, uint32_t argc, bool tail, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    if (info->entry_count == 1u) {
        if (!closure_info_selector_ready(info, err)) return false;
        const IdmBytecodeModule *callee_module = info->module;
        if (!callee_module) return idm_error_set(err, idm_span_unknown(NULL), "closure has no bytecode module");
        uint32_t entry = info->function_index;
        ...
        if (trivial && !fn->has_guard && fn->pattern_local_count == 0 && idm_arity_accepts(&fn->call_arity, argc)) {
            return enter_clause(vm, callee_module, entry, callee, NULL, NULL, 0, dst, arg_values, argc, NULL, tail, true, rewind_ip, status, out_reason, err);
        }
    }
    ClauseSelection selected;
    ClauseArgs args = clause_args_from(arg_values, argc);
    bool selected_ok = info->entry_count == 1u ?
                       select_ready_closure_clause(vm, callee, info, args, &selected, err) :
                       select_closure_clause(vm, callee, info, args, &selected, err);
    if (!selected_ok) return false;
    if (!selected.matched) {
        const IdmBytecodeModule *cm = info->module;
        const char *cname = NULL;
        uint32_t first_entry = info->entry_count > 1u && info->entries ? info->entries[0] : info->function_index;
        if (cm && first_entry < cm->function_count) cname = cm->functions[first_entry].name;
        uint32_t min = UINT32_MAX;
        uint32_t max = 0;
        bool arity_accepted = closure_info_accepts_arity(info, argc, &min, &max);
        bool budget_exhausted = selected.guard_budget_exhausted;
        clause_selection_destroy(&selected);
        if (budget_exhausted) return raise_guard_budget(cname, err);
        if (!arity_accepted && min != UINT32_MAX) return raise_call_arity(vm, cname, argc, min, max, err);
        return raise_no_clause(vm, cname, args, err);
    }
    bool entered = enter_clause(vm, NULL, selected.function_index, callee, NULL, NULL, 0, dst, arg_values, argc, selected.has_bindings ? &selected.bindings : NULL, tail, true, rewind_ip, status, out_reason, err);
    clause_selection_destroy(&selected);
    return entered;
}
...
static bool call_value(Vm *vm, uint32_t dst, IdmValue callee, const IdmValue *args, uint32_t argc, bool tail, size_t rewind_ip, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    IdmClosureInfo info;
    if (!idm_closure_info(callee, &info)) {
        {
            IdmBuffer dbg;
            idm_buf_init(&dbg);
            idm_value_write(&dbg, callee);
            idm_error_set(err, idm_span_unknown(NULL), "attempted to call a non-closure: %.*s", (int)(dbg.len > 80 ? 80 : dbg.len), dbg.data ? dbg.data : "");
            idm_buf_destroy(&dbg);
        }
        return idm_error_reason(vm->rt, err, "not-callable", 1, callee);
    }
    return call_closure_clause(vm, dst, callee, &info, args, argc, tail, rewind_ip, status, out_reason, err);
}
...
static bool closure_info_is_guard_free(const IdmClosureInfo *info) {
    const IdmBytecodeModule *m = info ? info->module : NULL;
    if (!m || info->entry_count == 0) return false;
    for (size_t i = 0; i < info->entry_count; i++) {
        uint32_t idx = info->entry_count == 1u ? info->function_index : info->entries[i];
...
static bool select_receive_message(Vm *vm, IdmValue closure, ReceiveCursor *cursor, ReceiveSelection *selection, IdmError *err) {
    ...
    IdmClosureInfo info;
    if (!idm_closure_info(closure, &info)) {
        receive_selection_destroy(selection);
        return idm_error_set(err, idm_span_unknown(NULL), "receive expects a function value");
    }
    selection->guard_free = closure_info_is_guard_free(&info);
    size_t index = 0;
    ...
    while (receive_cursor_next(cursor, &index, &msg)) {
        ...
        bool ok = select_closure_clause(vm, closure, &info, args, &candidate, err);
        if (!ok) {
            receive_selection_destroy(selection);
            return false;
        }
...
static bool op_recv(Vm *vm, Frame *frame, size_t instr_ip, const uint32_t *operands, IdmExecStatus *status, IdmValue *out_reason, IdmError *err) {
    ...
    if (selection.matched) {
        ReductionCharge charge;
        if (!vm_spend_reduction(vm, status, &charge, err)) {
            receive_selection_destroy(&selection);
            return false;
        }
        if (charge == REDUCTION_YIELD_BEFORE) {
        ...
        bool ok = enter_clause(vm, NULL, selection.clause.function_index, closure, NULL, NULL, 0, dst, &arg, 1u, selection.clause.has_bindings ? &selection.clause.bindings : NULL, tail, false, SIZE_MAX, status, out_reason, err);
        receive_selection_destroy(&selection);
        return ok;
    }
...
static bool op_make_record(Vm *vm, Frame *frame, size_t instr_ip, const uint32_t *operands, IdmError *err) {
    ...
    IDM_ASSERT_PROVED(instr_ip < module->code_site_count);
    const IdmBcRecordSite *site = &module->code_sites[instr_ip].record;
    IDM_ASSERT_PROVED(site->shape && site->field_count == field_count);
    ...
    for (uint32_t i = 0; i < field_count && ok; i++) {
        const IdmTypeTerm *contract = site->contracts ? site->contracts[i] : NULL;
        if (ok && contract && !idm_value_matches_type_term(field_values[i], contract)) {
            IdmSymbol *field = idm_record_shape_field_symbol(site->shape, i);
            IDM_ASSERT_PROVED(field);
            IdmBuffer expected;
            ...
            if (!idm_type_term_write(&expected, contract)) {
                ...
                ok = idm_error_set(err, idm_span_unknown(NULL), "record field '%s' expects %s, got %s", idm_symbol_text(field), expected.data ? expected.data : "_", idm_value_dispatch_type_name(field_values[i]));
                idm_buf_destroy(&expected);
            }
        }
    }
    ...
    if (ok) {
        record = idm_record_from_shape(vm->rt, site->shape, field_values, err);
        ok = ok && !(err && err->present);
    }
    if (!ok) return false;
...
static bool vm_run_loop_inner(Vm *vm, int64_t *budget, IdmExecStatus *status, IdmValue *out_result, IdmValue *out_reason, IdmError *err) {
    ...
    *status = IDM_EXEC_DONE;
    ...
    for (;;) {
        ...
        IDM_ASSERT_PROVED(instr_ip < module->code_count);
        IdmOpcode op = (IdmOpcode)module->code[instr_ip];
        IDM_ASSERT_PROVED((size_t)op < (size_t)IDM_OP_COUNT);
        const IdmOpcodeInfo *info = &idm_opcode_infos[op];
        ...
        if (info->count_operand != IDM_OPCODE_NO_COUNT) width += (size_t)operands[info->count_operand] * (size_t)info->repeat_width;
        ...
        switch (op) {
            ...
            case IDM_OP_MAKE_CLOSURE: {
                ...
                uint64_t cached_bits = atomic_load_explicit(&site->closure, memory_order_acquire);
                if (cached_bits != 0u) {
                    IdmValue cached;
                    cached.bits = cached_bits;
                    if (idm_closure_env(cached) == frame->env) {
                        if (!frame_reg_write(frame, dst, cached, err)) return false;
                        break;
                    }
                }
                IdmPatternSelector *selector = idm_bc_selector_at(module, instr_ip);
                IDM_ASSERT_PROVED(selector);
                bool try_cache = cached_bits == 0u;
                IdmHeap *prev_heap = try_cache ? idm_swap_active_heap(&rt->immortal) : NULL;
                IdmValue closure = idm_closure_multi_selectable_in_module(rt, module, &operand, 1u, selector, NULL, 0, frame->env, err);
                if (try_cache) idm_set_active_heap(prev_heap);
                if (err && err->present) return false;
                if (try_cache) {
                    uint64_t expected = 0u;
                    if (atomic_compare_exchange_strong_explicit(&site->closure, &expected, closure.bits, memory_order_acq_rel, memory_order_acquire) == false) {
                        IdmValue winner;
                        winner.bits = expected;
                        if (idm_closure_env(winner) == frame->env) closure = winner;
                    }
                }
            ...
            case IDM_OP_MAKE_CLOSURE_CAPTURES: {
                ...
                IdmPatternSelector *selector = idm_bc_selector_at(module, instr_ip);
                IDM_ASSERT_PROVED(selector);
                IdmValue closure = idm_closure_multi_selectable_in_module(rt, module, &fn_index, 1u, selector, capture_values, capture_count, frame->env, err);
                if (err && err->present) return false;
                if (!frame_reg_write(frame, dst, closure, err)) return false;
                break;
            }
            case IDM_OP_MAKE_MULTI_CLOSURE: {
                ...
                IdmPatternSelector *selector = idm_bc_selector_at(module, instr_ip);
                IDM_ASSERT_PROVED(selector);
                IdmValue closure = idm_closure_multi_selectable_in_module(rt, module, payload, entry_count, selector, capture_values, capture_count, frame->env, err);
                if (err && err->present) return false;
                if (!frame_reg_write(frame, dst, closure, err)) return false;
                break;
            }
            case IDM_OP_CALL:
                if (!execute_call_op(vm, frame, instr_ip, operands, status, out_reason, err)) return false;
                break;
            case IDM_OP_CALL_PRIMITIVE: {
                ...
                const IdmValue *args = NULL;
                if (!frame_reg_slice(frame, first_arg, argc, &args, err)) return false;
                if (!execute_primitive_clause(vm, primitive, dst, args, argc, true, instr_ip, status, out_reason, err)) return false;
                break;
            }
            case IDM_OP_MATCH:
            ...
            case IDM_OP_MAKE_RECORD:
                (void)payload;
                if (!op_make_record(vm, frame, instr_ip, operands, err)) return false;
                break;
            ...
            case IDM_OP_MATCH_BIND: {
                uint32_t fn_index = operands[0];
                uint32_t scrutinee_reg = operands[1];
                uint32_t first_dst = operands[2];
                uint32_t count = operands[3];
                IdmValue scrutinee = idm_nil();
                if (!frame_reg_value(frame, scrutinee_reg, &scrutinee, err)) return false;
                IDM_ASSERT_PROVED(fn_index < module->function_count);
                IdmPatternSelector *selector = idm_bc_function_selector_at(module, fn_index);
                IDM_ASSERT_PROVED(selector);
                ClauseSelection selected;
                ClauseArgs clause_args = clause_args_from(&scrutinee, 1u);
                ClauseGuardCtx guard_ctx;
                guard_ctx.vm = vm;
                guard_ctx.module = module;
                guard_ctx.closure = idm_nil();
                ...
                guard_ctx.exhausted = false;
                if (!run_selector(vm, selector, &guard_ctx, &selected, err)) return false;
                const IdmBcFunction *fn = &module->functions[fn_index];
                if (!selected.matched) {
                    bool budget_exhausted = selected.guard_budget_exhausted;
                    const char *bind_name = fn->name;
                    clause_selection_destroy(&selected);
                    if (budget_exhausted) return raise_guard_budget(bind_name, err);
                    return raise_no_clause(vm, bind_name, clause_args, err);
                }
                IDM_ASSERT_PROVED(count == fn->pattern_local_count);
                for (uint32_t i = 0; i < count; i++) {
                    uint32_t slot = fn->pattern_locals[i].slot;
                    IDM_ASSERT_PROVED(slot == i);
                    const IdmValue *bound = selected.has_bindings ? idm_pattern_bindings_get_slot(&selected.bindings, slot) : NULL;
                    if (!bound && selected.has_bindings) bound = idm_pattern_bindings_get(&selected.bindings, fn->pattern_locals[i].name);
                    if (!bound) {
                        clause_selection_destroy(&selected);
                        return idm_error_set(err, idm_span_unknown(NULL), "pattern binding %u missing in bind", slot);
                    }
                    if (!frame_reg_write(frame, first_dst + slot, *bound, err)) {
                        clause_selection_destroy(&selected);
                        return false;
                    }
                }
            ...
            case IDM_OP_LOAD_PACKAGE_SLOT: {
                ...
                if (!env) {
                    ...
                    atomic_store_explicit(&site->env, env, memory_order_release);
                }
            ...
            case IDM_OP_COUNT:
                IDM_ASSERT_PROVED(0 && "opcode count sentinel");
                break;
        }
...
static bool vm_run_loop(Vm *vm, int64_t *budget, IdmExecStatus *status, IdmValue *out_result, IdmValue *out_reason, IdmError *err) {
    bool result = vm_run_loop_handlers(vm, budget, status, out_result, out_reason, err);
    vm->budget = NULL;
    return result;
}
        ...
        if (vm->handler_count == 0) {
            ...
            if (err->present && err->span.line == 0) {
                ...
                if (top && top->module) {
                    ...
                    if (where.line != 0) idm_error_set_span(err, where);
                }
            }
            size_t depth = vm->frame_count;
            ...
            for (size_t i = 0; i < shown; i++) {
                ...
                idm_error_note_at(err, where, "in %s", fname && fname[0] ? fname : "<fn>");
            }
...
bool idm_vm_call_closure(IdmRuntime *rt, IdmValue closure, const IdmValue *args, uint32_t argc, IdmValue *out, IdmError *err) {
    ...
    const IdmBytecodeModule *module = info.module;
    if (!module) {
        idm_profile_scope_end(&prof);
...
bool idm_vm_call_closure_budgeted(IdmRuntime *rt, IdmValue closure, const IdmValue *args, uint32_t argc, int64_t budget, IdmValue *out, IdmError *err) {
    IdmClosureInfo info;
    if (!idm_closure_info(closure, &info)) return idm_error_set(err, idm_span_unknown(NULL), "call target must be a function value");
    const IdmBytecodeModule *module = info.module;
    ...
    VmClosureSetup setup = { closure, args, argc };
    IdmExecStatus status = IDM_EXEC_DONE;
    if (!vm_bootstrap_run_budgeted(rt, module, idm_vm_default_limits(), NULL, vm_setup_closure, &setup, budget, &status, "exited outside the scheduler", out, err)) return false;
    if (status != IDM_EXEC_DONE) return idm_error_set(err, idm_span_unknown(NULL), "call exceeded its budget of %lld reductions", (long long)budget);
    return true;
}
...
bool idm_exec_setup_thunk(IdmExec *exec, IdmValue closure, IdmError *err) {
    ...
    return call_value(exec, 0, closure, NULL, 0, false, SIZE_MAX, NULL, NULL, err);
}
