...
CC ?= cc
VERSION_MAJOR := 0
VERSION_MINOR := $(shell git rev-list --count HEAD 2>/dev/null || echo 0)
VERSION := $(VERSION_MAJOR).$(VERSION_MINOR)-dev
BUILD := build

CPPFLAGS ?= -D_POSIX_C_SOURCE=200809L -DIDM_VERSION=\"$(VERSION)\" -DIDM_VERSION_MAJOR=$(VERSION_MAJOR) -DIDM_VERSION_MINOR=$(VERSION_MINOR) -Iinclude
