...
VERSION_MAJOR := 0
VERSION_MINOR := 0
VERSION := $(VERSION_MAJOR).$(VERSION_MINOR)-dev
BUILD := build

CPPFLAGS ?= -D_POSIX_C_SOURCE=200809L -DIDM_VERSION=\"$(VERSION)\" -DIDM_VERSION_MAJOR=$(VERSION_MAJOR) -DIDM_VERSION_MINOR=$(VERSION_MINOR) -Iinclude -I$(BUILD)
...
BUILD_ID_HDR := $(BUILD)/idm_build_id.h
BUILD_ID_SRCS := $(sort $(LIB_SRCS) $(CLI_SRCS) $(wildcard include/idiom/*.h) src/expand/internal.h Makefile)
...
UNIT_CASE_SRCS := \
	...
	tests/unit/port_close_input.c \
	tests/unit/primitive_registry.c \
	tests/unit/signature_contract.c \
	tests/unit/type_term.c \
	tests/unit/infer.c \
	tests/unit/reader_artifact.c \
	tests/unit/reader_escape.c \
	tests/unit/reader_offset.c \
	tests/unit/json_write.c \
	tests/unit/repl.c \
	tests/unit/regex_set.c \
	tests/unit/env_slots.c \
	tests/unit/rope.c \
	tests/unit/string_case.c \
	tests/unit/syntax_equal.c \
	tests/unit/syntax_pretty.c \
	tests/unit/wire_helpers.c

UNIT_SRCS := $(UNIT_CASE_SRCS) tests/unit/runner.c

LANG_TESTS := tests/lang
PKG_TESTS := tests/pkg/bytecode_shape tests/pkg/cross_trait_order tests/pkg/forward_type_scope tests/pkg/reader_constructor_artifact tests/pkg/surface_macros
REAL_TESTS := tests/real
TUTORIAL_TESTS := tests/tutorial
TEST_PATHS := $(LANG_TESTS) $(PKG_TESTS) $(REAL_TESTS) $(TUTORIAL_TESTS)
...
.PHONY: all build release ish iem test check bench profile sanitize clean help install .release-test
.PHONY: .pty .real .roundtrip .bootstrap .profile-memcheck .profile-callgrind .profile-helgrind .asan .ubsan .tsan
...
IDIOMROOT ?= $(HOME)/idiom
STD_PACKAGES := $(filter-out kernel,$(notdir $(patsubst %/,%,$(wildcard std/*/))))

install: $(RELEASE_IDIOMC) ish
	test -n "$(IDIOMROOT)"
	test "$(abspath $(IDIOMROOT))" != "$(CURDIR)"
	install -d $(IDIOMROOT)/bin
	install -m 0755 $(RELEASE_IDIOMC) $(IDIOMROOT)/bin/idiomc
	install -m 0755 $(BUILD)/ish $(IDIOMROOT)/bin/ish
	rm -rf $(IDIOMROOT)/std $(IDIOMROOT)/app
	tar -c std app | tar -x -C $(IDIOMROOT)
	@set -e; for p in $(STD_PACKAGES); do \
		printf 'use std/%s\n:ok\n' $$p | IDIOMROOT=$(IDIOMROOT) $(IDIOMROOT)/bin/idiomc eval - > /dev/null; \
	done
	@set -e; for p in app/ish app/iem; do \
		printf 'use %s\n:ok\n' $$p | IDIOMROOT=$(IDIOMROOT) $(IDIOMROOT)/bin/idiomc eval - > /dev/null; \
	done
	@echo "installed idiomc, std, and app packages into $(IDIOMROOT), caches warm"

ISH_SRCS := $(wildcard app/ish/*.id)
IEM_SRCS := $(wildcard app/iem/*.id app/iem/*/*.id)

ish: $(BUILD)/ish

$(BUILD)/ish: $(RELEASE_IDIOMC) $(ISH_SRCS)
	$(RELEASE_IDIOMC) build app/ish -o $@

iem: $(BUILD)/iem

$(BUILD)/iem: $(RELEASE_IDIOMC) $(IEM_SRCS)
	$(RELEASE_IDIOMC) build app/iem/main -o $@

test check bench profile .profile-memcheck .profile-callgrind .profile-helgrind .real .roundtrip .bootstrap sanitize .asan .ubsan .tsan .release-test ish iem $(BUILD)/ish $(BUILD)/iem: export IDIOMROOT := $(CURDIR)

test: $(IDIOMC) $(UNIT)
	IDIOMC_UNDER_TEST=$(abspath $(IDIOMC)) $(UNIT)
	$(IDIOMC) test $(TEST_PATHS)

check: $(IDIOMC) $(RELEASE_IDIOMC) $(UNIT) $(BUILD)/pty_driver $(BUILD)/ish
	$(MAKE) test
	$(MAKE) .pty
	$(MAKE) .real
	$(MAKE) .roundtrip
...
$(BUILD)/pty_driver: tools/pty_driver.c
	mkdir -p $(BUILD)
	$(CC) $(DEBUG_CFLAGS) -o $@ tools/pty_driver.c

.pty: $(IDIOMC) $(BUILD)/pty_driver $(BUILD)/ish
	IDIOMROOT=$(CURDIR) sh tools/run_pty_tests.sh $(BUILD)/pty_driver $(IDIOMC)

bench: $(RELEASE_IDIOMC)
	$(RELEASE_IDIOMC) test -bench . -count $(BENCH_COUNT) tests/bench

.release-test: $(RELEASE_IDIOMC)
	$(RELEASE_IDIOMC) test $(TEST_PATHS)

profile:
	$(MAKE) .profile-memcheck
	$(MAKE) .profile-callgrind
	$(MAKE) .profile-helgrind

.profile-memcheck: $(RELEASE_IDIOMC)
	mkdir -p $(PROFILE_DIR)
	$(VALGRIND) --error-exitcode=99 --tool=memcheck --track-origins=yes --leak-check=full --show-leak-kinds=definite,possible --errors-for-leak-kinds=definite,possible --log-file=$(PROFILE_DIR)/memcheck.log $(RELEASE_IDIOMC) test -bench . -count 1 -benchtime $(PROFILE_COUNT)x -json tests/bench > $(PROFILE_DIR)/memcheck.json

.profile-callgrind: $(RELEASE_IDIOMC)
	mkdir -p $(PROFILE_DIR)
	$(VALGRIND) --error-exitcode=99 --tool=callgrind --collect-jumps=yes --cache-sim=yes --callgrind-out-file=$(PROFILE_DIR)/callgrind.out --log-file=$(PROFILE_DIR)/callgrind.log $(RELEASE_IDIOMC) test -bench . -count 1 -benchtime $(PROFILE_COUNT)x -json tests/bench > $(PROFILE_DIR)/callgrind.json

.profile-helgrind: $(RELEASE_IDIOMC)
	mkdir -p $(PROFILE_DIR)
	$(VALGRIND) --error-exitcode=99 --tool=helgrind --history-level=approx --log-file=$(PROFILE_DIR)/helgrind.log $(RELEASE_IDIOMC) test -bench . -count 1 -benchtime $(PROFILE_COUNT)x -json tests/bench > $(PROFILE_DIR)/helgrind.json

.real: $(IDIOMC)
	$(IDIOMC) run $(REAL_PROG)

.roundtrip: $(IDIOMC)
	$(IDIOMC) build $(REAL_PROG) -o $(BUILD)/roundtrip_actor_regex_port
	$(IDIOMC) run $(BUILD)/roundtrip_actor_regex_port
	$(IDIOMC) build $(BOOT_PKG) -o $(BUILD)/roundtrip_reader_constructor_artifact
	$(IDIOMC) run $(BUILD)/roundtrip_reader_constructor_artifact

.bootstrap: $(IDIOMC) $(RELEASE_IDIOMC)
	$(IDIOMC) build $(BOOT_PKG) -o $(BUILD)/bootstrap_debug_reader_constructor_artifact
	$(RELEASE_IDIOMC) build $(BOOT_PKG) -o $(BUILD)/bootstrap_release_reader_constructor_artifact
	$(IDIOMC) run $(BUILD)/bootstrap_debug_reader_constructor_artifact > $(BUILD)/bootstrap_debug.out
	$(RELEASE_IDIOMC) run $(BUILD)/bootstrap_release_reader_constructor_artifact > $(BUILD)/bootstrap_release.out
...
$(BUILD_ID_HDR): $(BUILD_ID_SRCS)
	@mkdir -p $(dir $@)
	@printf '#define IDM_BUILD_ID "%s"\n' "$$(cat $(BUILD_ID_SRCS) | sha256sum | cut -d ' ' -f 1)" > $@.tmp
	@if cmp -s $@.tmp $@; then rm $@.tmp; else mv $@.tmp $@; fi

$(foreach t,debug release asan ubsan tsan,$(BUILD)/obj/$(t)/src/expand/expand_packages.o): $(BUILD_ID_HDR)

$(foreach t,debug release asan ubsan tsan,$(BUILD)/obj/$(t)/src/common/common.o): Makefile
...
TUTORIAL.md: docs/tutorial/intro.md $(wildcard docs/tutorial/ch*.md)
	cat docs/tutorial/intro.md docs/tutorial/ch*.md > $@
